  ...
  wrap.innerHTML=`
    ...
    <article class="printReportPage familyReportPage parentMirrorPage">
      ...
      <section class="parentMirrorSection">
        <div class="parentMirrorSectionTitle"><span>↗</span><div><h2>Avance observado durante el ciclo</h2><p>Comparación entre la aplicación inicial y final.</p></div></div>
        <div class="followupPrintGrid">
          <div><b>Lectura</b><p>VE: ${esc(followup.lectura?.initial?.ve??"—")} → ${esc(followup.lectura?.final?.ve??"—")} · CL: ${esc(followup.lectura?.initial?.cl??"—")}% → ${esc(followup.lectura?.final?.cl??"—")}% · EL: ${esc(followup.lectura?.initial?.el??"—")} → ${esc(followup.lectura?.final?.el??"—")}</p></div>
          <div><b>Escritura</b><p>Redacción: ${esc(followup.escritura?.initial?.writing_total??"—")} → ${esc(followup.escritura?.final?.writing_total??"—")} · Ortografía de palabras: ${esc(followup.escritura?.initial?.words_score??"—")} → ${esc(followup.escritura?.final?.words_score??"—")}</p></div>
        </div>
      </section>`:""}
