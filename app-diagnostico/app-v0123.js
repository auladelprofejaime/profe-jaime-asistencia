...
async function startLecturaGroupTimer(){
  ...
  $("#lecturaTimerStatus").textContent="Lectura iniciada. Toca el número de lista en cuanto cada alumno termine.";
}
function captureLecturaStudentTime(studentId){
  ...
  if(prior){
    ...
    $("#lecturaTimerEditMessage").textContent=`El alumno ${student.list_number??"—"} ya tiene registrado ${lecturaFormatSeconds(prior.seconds)}. ¿Deseas modificar su tiempo?`;
  ...
  session.captures[String(studentId)]={seconds:lecturaTimerElapsed(session),capturedAt:Date.now(),listNumber:student.list_number,name:student.name};
  saveLecturaTimerSession(session);renderLecturaTimerGrid();
}
function saveLecturaTimerEdit(){
  ...
  session.captures[lecturaTimerEditStudentId]={...prior,seconds:min*60+sec,modifiedAt:Date.now()};
  saveLecturaTimerSession(session);$("#lecturaTimerEditModal").classList.add("hidden");lecturaTimerEditStudentId=null;renderLecturaTimerGrid();
}
...
function finishLecturaGroupTimer(){
  ...
  lecturaTimerStudents.forEach(s=>{
    const cap=captures[String(s.student_id)];if(!cap)return;
    lecturaSetPrefill(session.periodId,String(s.student_id),session.moment,{seconds:Number(cap.seconds),testCode:session.testCode,group:session.group,completedAt:Date.now()});
  });
  clearLecturaTimerSession();
  $("#lecturaTimerClock").textContent="00:00";renderLecturaTimerGrid();syncLecturaTimerControls();
  $("#lecturaTimerStatus").textContent=`✓ Aplicación finalizada. ${done} tiempos individuales quedaron listos para la captura de los 20 reactivos.`;
}
