...
async function loadLecturaTimerGroups(preferredGroup){
  ...
  const names=(groups||[]).map(g=>String(g.group_name||"")).filter(Boolean).filter(g=>g.toUpperCase()!=="MONITOR");
...
async function startLecturaGroupTimer(){
  ...
  $("#lecturaTimerStatus").textContent="Lectura iniciada. Toca los 2 últimos números del ID en cuanto cada alumno termine.";
}
function captureLecturaStudentTime(studentId){
  ...
  if(prior){
    ...
    $("#lecturaTimerEditMessage").textContent=`El alumno ID ${String(student.student_id||"").slice(-2)} ya tiene registrado ${lecturaFormatSeconds(prior.seconds)}. ¿Deseas modificar su tiempo?`;
