...
async function loadLecturaTimerGroups(preferredGroup){
  ...
  const groups=await rpc("teacher_diagnostic_groups",{p_period_id:activePeriod.id});
  const baseNames=(groups||[]).map(g=>String(g.group_name||"")).filter(Boolean).filter(g=>g.toUpperCase()!=="MONITOR");
  const names=[...new Set([...baseNames,...LECTURA_VESPERTINO_GROUPS])];
...
function renderLecturaTimerGroupTiles(names=[]){
  ...
  box.innerHTML=names.map(g=>`<button type="button" class="lecturaTimerGroupTile ${g===current?"selected":""}" data-timer-group="${esc(g)}"><b>Grupo ${esc(g)}</b><small>Abrir mosaico</small></button>`).join("");
...
async function loadLecturaTimerStudents(){
  ...
  const rows=localRows||await rpc("teacher_diagnostic_students",{p_period_id:activePeriod.id,p_group_name:group});
