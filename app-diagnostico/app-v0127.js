...
async function loadLecturaTimerGroups(preferredGroup){
  ...
  let groups=[];
  try{groups=await rpc("teacher_diagnostic_groups",{p_period_id:activePeriod.id})||[]}catch(e){console.warn("Grupos de diagnóstico no disponibles; usando padrón local de Lectura Eficaz.",e)}
  const baseNames=(groups||[]).map(g=>String(g.group_name||"")).filter(Boolean).filter(g=>g.toUpperCase()!=="MONITOR");
  const names=[...new Set([...LECTURA_VESPERTINO_GROUPS,...baseNames])];
...
async function openLecturaGroupTimer(sessionToResume=null){
  if(!activePeriod)return;
  $("#lecturaCard").classList.add("hidden");
  $("#studentCard").classList.add("hidden");
  $("#studentsCard").classList.add("hidden");
  ...
  await loadLecturaTimerGroups(s?.group||activeGroup);
