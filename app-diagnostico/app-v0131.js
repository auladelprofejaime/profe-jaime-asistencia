...
function lecturaAllRecoveredRows(group){
  ...
  return roster.map(st=>{
    ...
    Object.entries(pref).forEach(([k,v])=>{
      if(k.includes(`::${sid}::`) && v && (!v.group || v.group===group) && Number(v.seconds)>0)
        candidates.push({seconds:Number(v.seconds),when:Number(v.completedAt||0),source:"registro"});
    });
    hist.forEach(h=>{
      if(h?.group!==group)return;
      const c=h?.captures?.[sid];
      if(c && Number(c.seconds)>0)
        candidates.push({seconds:Number(c.seconds),when:Number(c.capturedAt||c.modifiedAt||h.finishedAt||0),source:"historial"});
    });
    candidates.sort((a,b)=>b.when-a.when);
    const best=candidates[0];
    return {...st,time:best?.seconds??null,completedAt:best?.when||null};
  }).filter(r=>r.time!==null).sort((a,b)=>Number(a.list_number)-Number(b.list_number));
}
function renderLecturaHistoryGroup(group){
  ...
  box.innerHTML=`<div class="sectionHead"><div><p class="eyebrow">Tiempos guardados</p><h3>Grupo ${esc(group)} · ${rows.length} registros</h3></div></div>
