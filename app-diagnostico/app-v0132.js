...
function openLecturaHistory(){
  ...
  const groups=LECTURA_VESPERTINO_GROUPS;
  const counts=groups.map(g=>[g,lecturaAllRecoveredRows(g).length]);
  $("#lecturaHistorySummary").textContent=`Registros encontrados en este dispositivo: ${counts.map(([g,n])=>`${g}: ${n}`).join(" · ")}`;
  $("#lecturaHistoryGroups").innerHTML=groups.map(g=>`<button type="button" class="lecturaTimerGroupTile" data-history-group="${esc(g)}"><b>Grupo ${esc(g)}</b><small>${lecturaAllRecoveredRows(g).length} tiempos guardados</small></button>`).join("");
