...
let diagnosticPeriods=[];
let lecturaTimerStudents=[];
let lecturaTimerTick=null;
let lecturaTimerEditStudentId=null;
const LECTURA_TIMER_SESSION_KEY="diagnosticLecturaGroupTimerSessionV1";
const LECTURA_TIMER_PREFILL_KEY="diagnosticLecturaGroupTimerPrefillsV1";
...
async function loadPeriods(){
  ...
  diagnosticPeriods=Array.isArray(rows)?rows:[];
  // Completa la fecha de creación directamente desde diagnostic_periods cuando la RPC no la devuelve.
  try{
    if(diagnosticPeriods.length){
      const ids=diagnosticPeriods.map(p=>p.id).filter(Boolean).join(",");
      const meta=await request(`/rest/v1/diagnostic_periods?select=id,created_at&id=in.(${encodeURIComponent(ids)})`);
      const byId=new Map((Array.isArray(meta)?meta:[]).map(x=>[String(x.id),x]));
      diagnosticPeriods=diagnosticPeriods.map(p=>({...p,created_at:p.created_at||byId.get(String(p.id))?.created_at||null}));
    }
  }catch(_e){}
  const box=$("#periodList");
  if(!diagnosticPeriods.length){box.className="list empty";box.textContent="Todavía no hay periodos diagnósticos.";return}
  const audit=lecturaAuditAllLocalTimes();
  const auditForPeriod=(id)=>audit.filter(r=>String(r.periodId)===String(id));
  const groupCounts=(arr)=>{const m={};arr.forEach(r=>{const g=r.group||"Sin grupo";m[g]=(m[g]||0)+1});return Object.entries(m).map(([g,n])=>`${esc(g)}: ${n}`).join(" · ")||"ninguno"};
  box.className="list";
  box.innerHTML=diagnosticPeriods.map(p=>{
    const pr=auditForPeriod(p.id);
    const mat=pr.filter(r=>lecturaAuditShift(r)==="Matutino");
    const ves=pr.filter(r=>lecturaAuditShift(r)==="Vespertino");
    const created=p.created_at?new Date(p.created_at).toLocaleDateString("es-MX",{day:"2-digit",month:"2-digit",year:"numeric"}):"fecha no disponible";
    return `<div class="row" style="align-items:flex-start">
      <div style="min-width:0;flex:1"><b>${esc(p.name)}</b><small>${esc(p.cycle)} · ${esc(p.shift)} · ${esc(p.status)} · ${p.student_count||0} alumnos</small>
        <div style="margin-top:8px;padding:9px 11px;border-radius:12px;background:#fff8df;font-size:.9em;line-height:1.45">
          <b>🔎 Auditoría de tiempos locales</b><br>
          Creado: <b>${esc(created)}</b><br>
          ☀️ Matutino: <b>${mat.length}</b> (${groupCounts(mat)})<br>
          🌙 Vespertino: <b>${ves.length}</b> (${groupCounts(ves)})
        </div>
      </div>
  ...
  box.querySelectorAll("[data-period]").forEach(b=>b.onclick=()=>openPeriod(diagnosticPeriods.find(p=>p.id===b.dataset.period)));
  box.querySelectorAll("[data-delete-period]").forEach(b=>b.onclick=()=>deletePeriod(diagnosticPeriods.find(p=>p.id===b.dataset.deletePeriod)));
}
...
async function openPeriod(p){
  ...
  try{
    ...
    renderLecturaVespertinoQuickAccess();
...
function lecturaFormatSeconds(total){
  total=Math.max(0,Math.floor(Number(total)||0));
  return `${String(Math.floor(total/60)).padStart(2,"0")}:${String(total%60).padStart(2,"0")}`;
}
function lecturaTimerSession(){
  try{return JSON.parse(localStorage.getItem(LECTURA_TIMER_SESSION_KEY)||"null")}catch{return null}
}
function saveLecturaTimerSession(session){
  localStorage.setItem(LECTURA_TIMER_SESSION_KEY,JSON.stringify(session));
}
function clearLecturaTimerSession(){
  localStorage.removeItem(LECTURA_TIMER_SESSION_KEY);
  if(lecturaTimerTick){clearInterval(lecturaTimerTick);lecturaTimerTick=null}
}
function lecturaPrefills(){
  try{return JSON.parse(localStorage.getItem(LECTURA_TIMER_PREFILL_KEY)||"{}")||{}}catch{return {}}
}
function saveLecturaPrefills(data){localStorage.setItem(LECTURA_TIMER_PREFILL_KEY,JSON.stringify(data))}
function lecturaPrefillKey(periodId,studentId,moment){return `${periodId}::${studentId}::${moment}`}
function lecturaGetPrefill(periodId,studentId,moment){return lecturaPrefills()[lecturaPrefillKey(periodId,studentId,moment)]||null}
function lecturaSetPrefill(periodId,studentId,moment,value){
  const data=lecturaPrefills();data[lecturaPrefillKey(periodId,studentId,moment)]=value;saveLecturaPrefills(data);
}
function lecturaRemovePrefill(periodId,studentId,moment){
  const data=lecturaPrefills();delete data[lecturaPrefillKey(periodId,studentId,moment)];saveLecturaPrefills(data);
}
function lecturaTimerElapsed(session=lecturaTimerSession()){
  if(!session?.startedAt)return 0;
  return Math.max(0,Math.floor((Date.now()-Number(session.startedAt))/1000));
}
function renderLecturaTimerTestOptions(){
  const moment=$("#lecturaTimerMoment").value||"initial";
  const options=lecturaOptionsForMoment(moment);
  const current=$("#lecturaTimerTest").value;
  $("#lecturaTimerTest").innerHTML=options.map(code=>`<option value="${code}">Prueba ${code} · ${esc(LECTURA_TESTS[code].title)}</option>`).join("");
  if(options.includes(current))$("#lecturaTimerTest").value=current;
}
async function loadLecturaTimerGroups(preferredGroup){
  if(!activePeriod)return;
  const names=[...LECTURA_VESPERTINO_GROUPS];
  $("#lecturaTimerGroup").innerHTML=names.map(g=>`<option value="${esc(g)}">Grupo ${esc(g)}</option>`).join("");
  const desired=preferredGroup&&names.includes(String(preferredGroup))?String(preferredGroup):(activeGroup&&names.includes(String(activeGroup))?String(activeGroup):names[0]);
  if(desired)$("#lecturaTimerGroup").value=desired;
  renderLecturaTimerGroupTiles(names);
  await loadLecturaTimerStudents();
}
function renderLecturaTimerGroupTiles(names=[]){
  const box=$("#lecturaTimerGroupTiles");
  if(!box)return;
  const current=$("#lecturaTimerGroup").value;
  box.innerHTML=names.map(g=>{const n=LECTURA_VESPERTINO_ROSTER[g]?.length;return `<button type="button" class="lecturaTimerGroupTile ${g===current?"selected":""}" data-timer-group="${esc(g)}"><b>Grupo ${esc(g)}</b><small>${n?`${n} alumnos · `:""}Abrir mosaico</small></button>`}).join("");
  box.querySelectorAll("[data-timer-group]").forEach(btn=>btn.onclick=async()=>{
    if(lecturaTimerSession()?.startedAt){alert("Finaliza la aplicación en curso antes de cambiar de grupo.");return}
    $("#lecturaTimerGroup").value=btn.dataset.timerGroup;
    renderLecturaTimerGroupTiles(names);
    await loadLecturaTimerStudents();
  });
}
async function loadLecturaTimerStudents(){
  const group=$("#lecturaTimerGroup").value;
  if(!group){lecturaTimerStudents=[];renderLecturaTimerGrid();return}
  const localRows=LECTURA_VESPERTINO_ROSTER[group];
  let rows=localRows;
  if(!rows){
    if(!activePeriod){lecturaTimerStudents=[];renderLecturaTimerGrid();return}
    try{rows=await rpc("teacher_diagnostic_students",{p_period_id:activePeriod.id,p_group_name:group})||[]}catch(e){rows=[];console.warn("No se pudieron cargar alumnos desde la nube.",e)}
  }
  lecturaTimerStudents=(rows||[]).filter(s=>String(s.student_id)!=="00001").sort((a,b)=>(Number(a.list_number)||999)-(Number(b.list_number)||999));
  if(localRows){const t=lecturaVespertinoTest(group);const sel=$("#lecturaTimerTest");sel.innerHTML=`<option value="${t.code}">${esc(t.title)}</option>`;sel.value=t.code;$("#lecturaTimerMoment").value="initial";}
  renderLecturaTimerGrid();
}
function renderLecturaTimerGrid(){
  const box=$("#lecturaTimerGrid");
  const session=lecturaTimerSession();
  const same=session&&activePeriod&&session.periodId===activePeriod.id&&session.group===$("#lecturaTimerGroup").value&&session.moment===$("#lecturaTimerMoment").value&&session.testCode===$("#lecturaTimerTest").value;
  const captures=same?(session.captures||{}):{};
  box.innerHTML=lecturaTimerStudents.map(s=>{
    const cap=captures[String(s.student_id)];
    const listNo=String(s.list_number??"—");
    return `<button type="button" class="lecturaTimerStudent ${cap?"captured":"pending"}" data-timer-student="${esc(s.student_id)}">
      <b>${cap?`✓ ${esc(listNo)} · ${lecturaFormatSeconds(cap.seconds)}`:esc(listNo)}</b>
      <small>${esc(s.name||"")}</small>
    </button>`;
  }).join("");
  box.querySelectorAll("[data-timer-student]").forEach(btn=>btn.onclick=()=>captureLecturaStudentTime(btn.dataset.timerStudent));
  const done=Object.keys(captures).length;
  $("#lecturaTimerCounter").textContent=lecturaTimerStudents.length?`${done} de ${lecturaTimerStudents.length} alumnos con tiempo registrado.`:"No hay alumnos disponibles en este grupo.";
}
function refreshLecturaTimerClock(){
  const session=lecturaTimerSession();
  if(!session?.startedAt){$("#lecturaTimerClock").textContent="00:00";return}
  $("#lecturaTimerClock").textContent=lecturaFormatSeconds(lecturaTimerElapsed(session));
}
function syncLecturaTimerControls(){
  const session=lecturaTimerSession();
  const running=!!session?.startedAt;
  ["#lecturaTimerGroup","#lecturaTimerMoment","#lecturaTimerTest"].forEach(sel=>$(sel).disabled=running);
  $("#startLecturaTimerBtn").disabled=running||!lecturaTimerStudents.length;
  $("#finishLecturaTimerBtn").disabled=!running;
  if(running){
    if(lecturaTimerTick)clearInterval(lecturaTimerTick);
    refreshLecturaTimerClock();
    lecturaTimerTick=setInterval(refreshLecturaTimerClock,250);
  }else if(lecturaTimerTick){clearInterval(lecturaTimerTick);lecturaTimerTick=null}
}
async function openLecturaGroupTimer(sessionToResume=null,preferredGroup=null){
  if(!activePeriod)return;
  $("#lecturaCard").classList.add("hidden");
  $("#studentCard").classList.add("hidden");
  $("#studentsCard").classList.add("hidden");
  $("#groupsCard").classList.add("hidden");
  $("#lecturaGroupTimerCard").classList.remove("hidden");
  const s=sessionToResume||lecturaTimerSession();
  $("#lecturaTimerMoment").value=s?.moment||$("#lecturaMoment").value||"initial";
  renderLecturaTimerTestOptions();
  if(s?.testCode && Array.from($("#lecturaTimerTest").options).some(o=>o.value===s.testCode))$("#lecturaTimerTest").value=s.testCode;
  await loadLecturaTimerGroups(preferredGroup||s?.group||activeGroup);
  renderLecturaTimerGrid();syncLecturaTimerControls();
  $("#lecturaTimerStatus").textContent=s?.startedAt?"Aplicación en curso recuperada. El cronómetro continúa desde la hora de inicio original.":"";
}
async function startLecturaGroupTimer(){
  if(!activePeriod||!lecturaTimerStudents.length)return;
  const existing=lecturaTimerSession();
  if(existing?.startedAt){alert("Ya hay una aplicación grupal en curso.");return}
  const session={version:1,periodId:activePeriod.id,periodName:activePeriod.name||"",group:$("#lecturaTimerGroup").value,moment:$("#lecturaTimerMoment").value,testCode:$("#lecturaTimerTest").value,startedAt:Date.now(),captures:{}};
  saveLecturaTimerSession(session);renderLecturaTimerGrid();syncLecturaTimerControls();
  $("#lecturaTimerStatus").textContent="Lectura iniciada. Toca el número de lista de cada alumno en cuanto termine.";
}
function captureLecturaStudentTime(studentId){
  const session=lecturaTimerSession();
  if(!session?.startedAt){alert("Primero toca “Iniciar lectura”.");return}
  const student=lecturaTimerStudents.find(s=>String(s.student_id)===String(studentId));
  if(!student)return;
  const prior=session.captures?.[String(studentId)];
  if(prior){
    lecturaTimerEditStudentId=String(studentId);
    $("#lecturaTimerEditMessage").textContent=`El alumno de lista ${student.list_number??"—"} ya tiene registrado ${lecturaFormatSeconds(prior.seconds)}. ¿Deseas modificar su tiempo?`;
    $("#lecturaTimerEditMinutes").value=Math.floor(Number(prior.seconds)/60);
    $("#lecturaTimerEditSeconds").value=Number(prior.seconds)%60;
    $("#lecturaTimerEditModal").classList.remove("hidden");
    return;
  }
  session.captures=session.captures||{};
  const seconds=lecturaTimerElapsed(session);
  session.captures[String(studentId)]={seconds,capturedAt:Date.now(),listNumber:student.list_number,name:student.name};
  saveLecturaTimerSession(session);
  lecturaSetPrefill(session.periodId,String(studentId),session.moment,{seconds,testCode:session.testCode,group:session.group,completedAt:Date.now()});
  renderLecturaTimerGrid();
}
function saveLecturaTimerEdit(){
  const session=lecturaTimerSession();if(!session||!lecturaTimerEditStudentId)return;
  const min=Number($("#lecturaTimerEditMinutes").value||0),sec=Number($("#lecturaTimerEditSeconds").value||0);
  if(!Number.isInteger(min)||min<0||!Number.isInteger(sec)||sec<0||sec>59||min*60+sec<=0){alert("Captura un tiempo válido.");return}
  session.captures=session.captures||{};
  const prior=session.captures[lecturaTimerEditStudentId]||{};
  const total=min*60+sec;
  session.captures[lecturaTimerEditStudentId]={...prior,seconds:total,modifiedAt:Date.now()};
  saveLecturaTimerSession(session);
  lecturaSetPrefill(session.periodId,lecturaTimerEditStudentId,session.moment,{seconds:total,testCode:session.testCode,group:session.group,completedAt:Date.now()});
  $("#lecturaTimerEditModal").classList.add("hidden");lecturaTimerEditStudentId=null;renderLecturaTimerGrid();
}
function cancelLecturaTimerEdit(){$("#lecturaTimerEditModal").classList.add("hidden");lecturaTimerEditStudentId=null}
const LECTURA_TIMER_HISTORY_KEY="diagnosticLecturaGroupTimerHistoryV1";
function lecturaTimerHistory(){try{return JSON.parse(localStorage.getItem(LECTURA_TIMER_HISTORY_KEY)||"[]")||[]}catch{return []}}
function saveLecturaTimerHistory(rows){localStorage.setItem(LECTURA_TIMER_HISTORY_KEY,JSON.stringify(rows))}
function archiveLecturaTimerSession(session){
  if(!session?.captures)return;
  const hist=lecturaTimerHistory();
  hist.unshift({...session,finishedAt:Date.now()});
  saveLecturaTimerHistory(hist.slice(0,100));
}
function lecturaRecoveredRows(group){
  const roster=LECTURA_VESPERTINO_ROSTER[group]||[];
  const pref=lecturaPrefills();
  const periodId=activePeriod?.id;
  return roster.map(st=>{
    let v=periodId?pref[lecturaPrefillKey(periodId,String(st.student_id),"initial")]:null;
    if(!v){
      const suffix=`::${st.student_id}::initial`;
      const matches=Object.entries(pref).filter(([k,val])=>k.endsWith(suffix)&&val?.group===group).sort((a,b)=>(b[1]?.completedAt||0)-(a[1]?.completedAt||0));
      v=matches[0]?.[1]||null;
    }
    return {...st,time:v?.seconds??null,completedAt:v?.completedAt||null};
  }).filter(r=>r.time!==null).sort((a,b)=>Number(a.list_number)-Number(b.list_number));
}

function lecturaPeriodLabel(periodId){
  const p=(diagnosticPeriods||[]).find(x=>String(x.id)===String(periodId));
  if(p)return `${p.name||"Periodo"}${p.cycle?` · ${p.cycle}`:""}${p.shift?` · ${p.shift}`:""}`;
  return periodId?`Periodo ${periodId}`:"Periodo no identificado";
}
function lecturaAllRecoveredRows(group){
  const roster=LECTURA_VESPERTINO_ROSTER[group]||[];
  const byId=new Map(roster.map(st=>[String(st.student_id),st]));
  const pref=lecturaPrefills();
  const hist=lecturaTimerHistory();
  const rows=[];

  Object.entries(pref).forEach(([k,v])=>{
    if(!v || Number(v.seconds)<=0)return;
    const parts=String(k).split("::");
    if(parts.length<3)return;
    const periodId=parts[0], sid=parts[1], moment=parts.slice(2).join("::");
    const st=byId.get(String(sid));
    if(!st)return;
    if(v.group && v.group!==group)return;
    rows.push({...st,time:Number(v.seconds),completedAt:Number(v.completedAt||0),periodId,
      periodLabel:lecturaPeriodLabel(periodId),moment,testCode:v.testCode||"",source:"registro"});
  });

  hist.forEach(h=>{
    if(h?.group!==group || !h?.captures)return;
    Object.entries(h.captures).forEach(([sid,c])=>{
      const st=byId.get(String(sid));
      if(!st || !c || Number(c.seconds)<=0)return;
      const periodId=String(h.periodId||"");
      const when=Number(c.capturedAt||c.modifiedAt||h.finishedAt||0);
      // Avoid duplicate row when the same captured time already exists as prefill.
      const dup=rows.some(r=>String(r.student_id)===String(sid) &&
        String(r.periodId)===periodId && Number(r.time)===Number(c.seconds) &&
        Math.abs(Number(r.completedAt||0)-when)<3000);
      if(!dup) rows.push({...st,time:Number(c.seconds),completedAt:when,periodId,
        periodLabel:lecturaPeriodLabel(periodId),moment:h.moment||"",testCode:h.testCode||"",source:"historial"});
    });
  });
  return rows.sort((a,b)=>(Number(b.completedAt||0)-Number(a.completedAt||0)) ||
    (Number(a.list_number)-Number(b.list_number)));
}
function lecturaDateTime(ts){
  if(!Number(ts))return "Sin fecha";
  try{return new Date(Number(ts)).toLocaleString("es-MX",{dateStyle:"medium",timeStyle:"short"})}
  catch{return new Date(Number(ts)).toLocaleString("es-MX")}
}
function renderLecturaHistoryGroup(group){
  const rows=lecturaAllRecoveredRows(group);
  const box=$("#lecturaHistoryResults");
  if(!rows.length){
    box.innerHTML=`<div class="status">No encontré tiempos almacenados para el grupo ${esc(group)} en este navegador.</div>`;
    return;
  }
  const buckets=new Map();
  rows.forEach(r=>{
    const day=r.completedAt?new Date(r.completedAt).toLocaleDateString("es-MX"):"Sin fecha";
    const key=`${r.periodId||"sin-periodo"}||${day}`;
    if(!buckets.has(key))buckets.set(key,{periodLabel:r.periodLabel,day,rows:[]});
    buckets.get(key).rows.push(r);
  });
  box.innerHTML=[...buckets.values()].map(b=>`
    <div class="card" style="margin:12px 0;padding:14px">
      <div class="sectionHead"><div>
        <p class="eyebrow">${esc(b.periodLabel)}</p>
        <h3>Grupo ${esc(group)} · ${esc(b.day)} · ${b.rows.length} registro${b.rows.length===1?"":"s"}</h3>
      </div></div>
      <div style="overflow:auto"><table style="width:100%;border-collapse:collapse">
        <thead><tr><th style="text-align:left;padding:8px">Lista</th><th style="text-align:left;padding:8px">Alumno</th>
        <th style="text-align:left;padding:8px">Tiempo</th><th style="text-align:left;padding:8px">Registrado</th></tr></thead>
        <tbody>${b.rows.sort((a,b)=>Number(a.list_number)-Number(b.list_number)).map(r=>`<tr>
          <td style="padding:8px;border-top:1px solid #eee"><b>${esc(r.list_number)}</b></td>
          <td style="padding:8px;border-top:1px solid #eee">${esc(r.name)}</td>
          <td style="padding:8px;border-top:1px solid #eee"><b>${lecturaFormatSeconds(r.time)}</b></td>
          <td style="padding:8px;border-top:1px solid #eee">${esc(lecturaDateTime(r.completedAt))}</td>
        </tr>`).join("")}</tbody></table></div>
    </div>`).join("");
}
function lecturaAuditAllLocalTimes(){
  const pref=lecturaPrefills();
  const hist=lecturaTimerHistory();
  const rows=[];
  Object.entries(pref).forEach(([k,v])=>{
    ...
    const parts=String(k).split("::");
    if(parts.length<3)return;
    const periodId=parts[0],studentId=parts[1],moment=parts.slice(2).join("::");
    rows.push({periodId,studentId,moment,group:v.group||"Sin grupo",seconds:Number(v.seconds),when:Number(v.completedAt||0),source:"registro"});
  });
  hist.forEach(h=>{
    if(!h?.captures)return;
    Object.entries(h.captures).forEach(([studentId,c])=>{
      if(!c || Number(c.seconds)<=0)return;
      const periodId=String(h.periodId||"");
      const when=Number(c.capturedAt||c.modifiedAt||h.finishedAt||0);
      const dup=rows.some(r=>r.studentId===String(studentId)&&r.periodId===periodId&&r.seconds===Number(c.seconds)&&Math.abs((r.when||0)-when)<3000);
      if(!dup)rows.push({periodId,studentId:String(studentId),moment:h.moment||"",group:h.group||"Sin grupo",seconds:Number(c.seconds),when,source:"historial",name:c.name||"",listNumber:c.listNumber??""});
    });
  });
  return rows;
}
function lecturaAuditShift(row){
  return String(row.studentId||"").startsWith("V")?"Vespertino":"Matutino";
}
function renderLecturaPeriodAudit(){
  const rows=lecturaAuditAllLocalTimes();
  const box=$("#lecturaHistoryResults");
  if(!rows.length){
    box.innerHTML='<div class="notice">No encontré tiempos del cronómetro guardados localmente en este navegador. No borres periodos todavía.</div>';
    return;
  }
  const periods=new Map();
  rows.forEach(r=>{
    const key=r.periodId||"sin-periodo";
    if(!periods.has(key))periods.set(key,[]);
    periods.get(key).push(r);
  });
  box.innerHTML=[...periods.entries()].map(([periodId,pr])=>{
    const p=(diagnosticPeriods||[]).find(x=>String(x.id)===String(periodId));
    const label=p?`${p.name||"Periodo"}${p.created_at?` · creado ${new Date(p.created_at).toLocaleDateString("es-MX")}`:""}`:`Periodo ${periodId}`;
    const mat=pr.filter(r=>lecturaAuditShift(r)==="Matutino");
    const ves=pr.filter(r=>lecturaAuditShift(r)==="Vespertino");
    const detail=(arr)=>{
      const groups={};arr.forEach(r=>groups[r.group]=(groups[r.group]||0)+1);
      return Object.keys(groups).length?Object.entries(groups).map(([g,n])=>`${esc(g)}: <b>${n}</b>`).join(" · "):"0";
    };
    const dates=pr.map(r=>r.when).filter(Boolean).sort((a,b)=>a-b);
    return `<div class="card" style="margin:12px 0;padding:14px">
      <p class="eyebrow">${esc(label)}</p>
      <h3>${pr.length} tiempo${pr.length===1?"":"s"} encontrado${pr.length===1?"":"s"}</h3>
      <p><b>☀️ Matutino:</b> ${mat.length} · ${detail(mat)}</p>
      <p><b>🌙 Vespertino:</b> ${ves.length} · ${detail(ves)}</p>
      <p class="muted">${dates.length?`Registros entre ${esc(lecturaDateTime(dates[0]))} y ${esc(lecturaDateTime(dates[dates.length-1]))}.`:"Sin fecha disponible."}</p>
      <p class="muted" style="font-size:.85em">ID del periodo: ${esc(periodId)}</p>
    </div>`;
  }).join("");
}
function openLecturaHistory(){
  hideAllWorkCards();
  $("#lecturaHistoryCard").classList.remove("hidden");
  const rows=lecturaAuditAllLocalTimes();
  const mat=rows.filter(r=>lecturaAuditShift(r)==="Matutino").length;
  const ves=rows.filter(r=>lecturaAuditShift(r)==="Vespertino").length;
  $("#lecturaHistorySummary").innerHTML=`<b>Auditoría antes de borrar periodos</b><br>☀️ Matutino: ${mat} tiempos · 🌙 Vespertino: ${ves} tiempos. Esta pantalla no modifica ni elimina nada.`;
  $("#lecturaHistoryGroups").innerHTML=`<button type="button" class="lecturaTimerGroupTile" id="auditPeriodsBtn"><b>🔎 Ver por periodo</b><small>Identifica exactamente en cuál quedaron los tiempos</small></button>`+
    LECTURA_VESPERTINO_GROUPS.map(g=>`<button type="button" class="lecturaTimerGroupTile" data-history-group="${esc(g)}"><b>🌙 Grupo ${esc(g)}</b><small>${lecturaAllRecoveredRows(g).length} tiempos vespertinos</small></button>`).join("");
  $("#auditPeriodsBtn").onclick=renderLecturaPeriodAudit;
  $("#lecturaHistoryGroups").querySelectorAll("[data-history-group]").forEach(b=>b.onclick=()=>renderLecturaHistoryGroup(b.dataset.historyGroup));
  renderLecturaPeriodAudit();
}
function showLecturaTimerResults(){
  const group=$("#lecturaTimerGroup").value||"3B";
  const rows=lecturaAllRecoveredRows(group);
  const panel=$("#lecturaTimerResults");
  panel.classList.remove("hidden");
  if(!rows.length){panel.innerHTML=`<div class="status">No encontré tiempos guardados para ${esc(group)} en este navegador.</div>`;return}
  panel.innerHTML=`<div class="sectionHead"><div><p class="eyebrow">Tiempos recuperados</p><h3>Grupo ${esc(group)} · ${rows.length} registros</h3></div><button type="button" class="secondary" id="closeLecturaResults">Cerrar</button></div>
  <div style="overflow:auto"><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:8px">Lista</th><th style="text-align:left;padding:8px">Alumno</th><th style="text-align:left;padding:8px">Tiempo</th></tr></thead><tbody>${rows.map(r=>`<tr><td style="padding:8px;border-top:1px solid #ddd"><b>${esc(r.list_number)}</b></td><td style="padding:8px;border-top:1px solid #ddd">${esc(r.name)}</td><td style="padding:8px;border-top:1px solid #ddd"><b>${lecturaFormatSeconds(r.time)}</b></td></tr>`).join("")}</tbody></table></div>`;
  $("#closeLecturaResults").onclick=()=>panel.classList.add("hidden");
}
function finishLecturaGroupTimer(){
  const session=lecturaTimerSession();if(!session?.startedAt)return;
  const missing=lecturaTimerStudents.filter(s=>!session.captures?.[String(s.student_id)]);
  const done=lecturaTimerStudents.length-missing.length;
  let message=`${done} de ${lecturaTimerStudents.length} alumnos con tiempo registrado.`;
  if(missing.length)message+=`\n\nFaltan: ${missing.map(s=>s.list_number??"—").join(", ")}.\n\n¿Deseas finalizar de todos modos?`;
  else message+="\n\n¿Deseas finalizar la aplicación?";
  if(!confirm(message))return;
  archiveLecturaTimerSession(session);
  clearLecturaTimerSession();
  $("#lecturaTimerClock").textContent="00:00";renderLecturaTimerGrid();syncLecturaTimerControls();
  $("#lecturaTimerStatus").textContent=`✓ Aplicación finalizada. ${done} tiempos individuales ya quedaron asociados a sus registros de Lectura Eficaz.`;
}
async function offerLecturaTimerResume(){
  const s=lecturaTimerSession();if(!s?.startedAt||!accessToken)return;
  const p=diagnosticPeriods.find(x=>x.id===s.periodId);if(!p)return;
  if(confirm("Hay una aplicación de Lectura Eficaz en curso. ¿Deseas continuar?")){
    await openPeriod(p);await openLecturaGroupTimer(s);
  }
}
...
function loadLecturaApplicationIntoForm(){
  ...
  if(saved){
    ...
    const prefill=activePeriod&&activeStudentBundle?lecturaGetPrefill(activePeriod.id,activeStudentBundle.student.id,moment):null;
    if(prefill?.seconds>0){
      const total=Number(prefill.seconds);
      $("#lecturaMinutes").value=Math.floor(total/60);
      $("#lecturaSeconds").value=total%60;
      if(prefill.testCode && Array.from($("#lecturaTestCode").options).some(o=>o.value===prefill.testCode)){
        $("#lecturaTestCode").value=prefill.testCode;renderLecturaMeta();
      }
      $("#lecturaStatus").textContent=`⏱ Tiempo grupal precargado: ${lecturaFormatSeconds(total)}. Captura ahora las 20 respuestas.`;
    }else{
...
async function saveLectura(){
  try{
    ...
    lecturaRemovePrefill(activePeriod.id,activeStudentBundle.student.id,moment);
    showLecturaResult(r);
...
$("#openLecturaGroupTimerBtn").onclick=()=>openLecturaGroupTimer();
$("#openLecturaGroupTimerFromGroupBtn").onclick=()=>openLecturaGroupTimer();
$("#backFromLecturaGroupTimer").onclick=()=>{
  $("#lecturaGroupTimerCard").classList.add("hidden");
  if(activeStudentBundle){$("#lecturaCard").classList.remove("hidden")}else if(activeGroup){$("#studentsCard").classList.remove("hidden")}else $("#groupsCard").classList.remove("hidden");
};
$("#lecturaTimerGroup").onchange=loadLecturaTimerStudents;
$("#viewLecturaTimerResultsBtn").onclick=showLecturaTimerResults;
$("#openLecturaHistoryBtn").onclick=openLecturaHistory;
$("#backFromLecturaHistory").onclick=()=>{hideAllWorkCards();$("#groupsCard").classList.remove("hidden");};

$("#lecturaTimerMoment").onchange=()=>{renderLecturaTimerTestOptions();renderLecturaTimerGrid()};
$("#lecturaTimerTest").onchange=renderLecturaTimerGrid;
$("#startLecturaTimerBtn").onclick=startLecturaGroupTimer;
$("#finishLecturaTimerBtn").onclick=finishLecturaGroupTimer;
$("#cancelLecturaTimerEdit").onclick=cancelLecturaTimerEdit;
$("#saveLecturaTimerEdit").onclick=saveLecturaTimerEdit;
