...
Profr. Jaime Armando Pérez Vázquez`;
  const bodyHtml=`<div class="email-compose">
    ...
    <div class="pdf-email-note"><b>El reporte se compartirá como archivo PDF.</b><br>
    Al continuar, se abrirá el menú de compartir del iPad. Selecciona Gmail. Por seguridad de iPadOS, Gmail podría pedirte pegar manualmente el correo del alumno; la app intentará copiarlo al portapapeles.</div>
    <div class="actions">
      <button id="sharePdfEmail" class="primary" type="button">Adjuntar PDF y abrir Gmail</button>
    </div>
  </div>`;
  showDialog('Enviar reporte PDF por correo',bodyHtml);
  $('#sharePdfEmail').onclick=async()=>{
    ...
    const report=await buildTitularPdfBlob();
    if(!report)return;
    try{
      if(navigator.clipboard){
        try{await navigator.clipboard.writeText(to)}catch(_){}
      }
...
Destinatario: ${to}`,
        ...
        alert('Este iPad no permite adjuntar el PDF directamente desde el navegador. Usa Generar PDF, guárdalo y adjúntalo desde Gmail.');
      }
    }catch(e){
      if(e.name!=='AbortError')alert('No se pudo abrir el menú para compartir. Genera el PDF y adjúntalo manualmente desde Gmail.');
    }
  };
}


async function buildTitularPdfBlob(){
  let sid=$('#titStudent').value,wid=$('#titSavedWeek').value;
  if(!sid||!wid){alert('Selecciona alumno y semana.');return null}
  let s=await req(store('students').get(sid)),
      w=await req(store('titularWeeks').get(wid)),
      rec=(await all('titularRecords')).filter(x=>x.weekId===wid&&x.studentId===sid),
      map=new Map(rec.map(x=>[x.subject,x]));
  if(!pdfReady())return alert('No se cargó el generador de PDF. Conéctate a internet, pulsa Actualizar app y vuelve a intentarlo.');
  let {jsPDF}=window.jspdf,doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
  pdfHeader(doc,'Seguimiento académico 3.º A');
  doc.setFontSize(10);
  doc.setFont('helvetica','bold');
  doc.text('Titular: Profr. Jaime Armando Pérez Vázquez',14,33);
  doc.setFont('helvetica','normal');
  doc.text(`Alumno: ${s.name||s.id}`,14,39);
  doc.text(`Grupo: 3.º A    No. de lista: ${s.number}    Semana: ${w.label}`,14,45);
  doc.setFontSize(9);
  doc.setTextColor(20,80,30);
  doc.text('VERDE: al corriente.',14,54);
  doc.setTextColor(150,110,0);
  doc.text('AMARILLO: requiere acercarse con el docente para revisar pendientes.',14,60);
  doc.setTextColor(180,20,20);
  let lines=doc.splitTextToSize('ROJO - ATENCIÓN URGENTE: el alumno debe acercarse cuanto antes con el docente para revisar su situación y confirmar si existe posibilidad de ponerse al corriente. La recepción de trabajos atrasados depende de los criterios, fechas y autorización de cada maestro.',178);
  doc.text(lines,14,66);
  doc.setTextColor(80);
  let y=66+lines.length*4.2;
  doc.text('BLANCO: no se recibió información de la asignatura durante esta semana.',14,y);
  y+=8;
  const interpretation=st=>st==='green'?'Al corriente':st==='yellow'?'Requiere revisar pendientes con el docente':st==='red'?'ATENCIÓN URGENTE: acercarse con el docente':'Sin información esta semana';
  let body=subjects.map(sub=>{let st=map.get(sub)?.status||'white';return [sub,st==='green'?'Verde':st==='yellow'?'Amarillo':st==='red'?'Rojo':'Sin información',interpretation(st)]});
  doc.autoTable({startY:y,head:[['Asignatura','Estado','Interpretación']],body,styles:{fontSize:8,cellPadding:2},headStyles:{fillColor:[245,196,0],textColor:[33,27,18]},columnStyles:{0:{cellWidth:50},1:{cellWidth:32},2:{cellWidth:98}}});
  y=doc.lastAutoTable.finalY+8;
  doc.setTextColor(0);
  doc.setFont('helvetica','bold');
  doc.text('Observaciones generales:',14,y);
  doc.setFont('helvetica','normal');
  doc.text(doc.splitTextToSize(map.get('OBS')?.text||'Sin observaciones.',145),55,y);
  y+=14;
  doc.setFillColor(255,230,230);
  doc.rect(12,y-5,186,22,'F');
  doc.setTextColor(150,20,20);
  doc.setFont('helvetica','bold');
  doc.text(doc.splitTextToSize('IMPORTANTE: Este reporte es informativo. Cada docente determina si recibe actividades atrasadas, bajo qué condiciones y dentro de qué plazo. Un estado rojo no garantiza que los trabajos atrasados sean aceptados.',178),16,y);
  y+=27;
  doc.setFillColor(255,248,220);
  doc.rect(12,y-5,186,22,'F');
  doc.setTextColor(90,65,0);
  doc.text(doc.splitTextToSize('AVISO: Este reporte no sustituye la información oficial publicada en Systelar. Se recomienda revisar constantemente dicha plataforma para consultar avisos, calificaciones, pendientes y actualizaciones.',178),16,y);
  pdfFooter(doc);
  return {
    blob:pdfBlob(doc),
    fileName:`seguimiento_3A_${(s.name||s.id).replace(/[^a-z0-9]+/gi,'_')}_${w.label.replace(/[^a-z0-9]+/gi,'_')}.pdf`,
    student:s,
    week:w,
    doc
  };
...
async function printTitular(){
  const report=await buildTitularPdfBlob();
  if(!report)return;
  const {student:s,week:w,blob,fileName}=report;
  let html=`<p><b>Titular:</b> Profr. Jaime Armando Pérez Vázquez</p>
  <p><b>Alumno:</b> ${safe(s.name||s.id)}</p>
  <p><b>Semana:</b> ${safe(w.label)}</p>
  <p>El PDF incluye el desglose por asignatura, significado de colores, observaciones y el aviso de Systelar.</p>`;
  printContent('Seguimiento académico 3.º A',html,blob,fileName);
}
...
async function snapshotData(){await ensureDB();let data={version:'5.4',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])data[n]=await all(n);return data}
