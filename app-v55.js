...
const REQUIRED_STORES={students:'id',attendance:'key',activities:'id',activityRecords:'key',titularWeeks:'id',titularRecords:'key',settings:'key',internalBackups:'id',methodologies:'id'};
...
function resetMethodologyForm(){$('#methodologyForm').reset();$('#metSubject').value='Español';$('#metMonth').value='';$('#editMethodologyId').value='';$('#methodologyFormTitle').textContent='Nueva metodología';$('#cancelMethodologyEdit').classList.add('hidden');$('#criteriaEditor').innerHTML='';addCriterion({name:'Actividades y tareas',percent:100})}
async function saveMethodology(e){e.preventDefault();let criteria=getCriteriaFromEditor(),total=criteria.reduce((s,c)=>s+c.percent,0);if(!criteria.length)return alert('Agrega al menos un criterio.');if(Math.abs(total-100)>0.001)return alert('Los porcentajes deben sumar exactamente 100%.');let id=$('#editMethodologyId').value||crypto.randomUUID(),old=await req(store('methodologies').get(id));let m={id,shift:$('#metShift').value,group:$('#metGroup').value,name:norm($('#metName').value),subject:norm($('#metSubject').value)||'Español',month:norm($('#metMonth').value),criteria,assignments:old?.assignments||{},created:old?.created||new Date().toISOString(),updated:new Date().toISOString()};await put('methodologies',m);resetMethodologyForm();await renderMethodologies();await refreshCalculationMethodologies();alert('Metodología guardada.')}
async function editMethodology(id){let m=await req(store('methodologies').get(id));if(!m)return;$('#editMethodologyId').value=m.id;$('#metShift').value=[...$('#metShift').options].find(o=>sameShift(o.value,m.shift))?.value||m.shift;await fillGroups('met');$('#metGroup').value=[...$('#metGroup').options].find(o=>sameGroup(o.value,m.group))?.value||m.group;$('#metName').value=m.name;$('#metSubject').value=m.subject||'Español';$('#metMonth').value=m.month||'';$('#criteriaEditor').innerHTML='';m.criteria.forEach(addCriterion);$('#methodologyFormTitle').textContent='Editar metodología';$('#cancelMethodologyEdit').classList.remove('hidden');pane('met','manage');$('#methodologyForm').scrollIntoView({behavior:'smooth'})}
async function renderMethodologies(){if(!db)return;let list=(await all('methodologies')).sort((a,b)=>a.name.localeCompare(b.name,'es',{sensitivity:'base'})),box=$('#methodologiesList');if(!list.length){box.className='list empty';box.textContent='No hay metodologías.';return}box.className='list';box.innerHTML=list.map(m=>`<div class="row"><div><strong>${safe(m.name)}</strong><small>${safe(m.subject||'Español')} · ${safe(m.month||'Mes sin definir')} · ${safe(m.shift)} · Grupo ${safe(m.group)} · ${m.criteria.map(c=>`${safe(c.name)} ${c.percent}%`).join(' · ')}</small></div><div class="rowactions"><button class="edit" data-metedit="${m.id}">Editar</button><button class="del" data-metdel="${m.id}">Eliminar</button></div></div>`).join('');$$('[data-metedit]').forEach(b=>b.onclick=()=>editMethodology(b.dataset.metedit));$$('[data-metdel]').forEach(b=>b.onclick=async()=>{if(confirm('¿Eliminar esta metodología? Las actividades y calificaciones no se borrarán.')){await del('methodologies',b.dataset.metdel);renderMethodologies();refreshCalculationMethodologies()}})}
async function refreshMethodologyUI(){if(!$('#criteriaEditor').children.length)resetMethodologyForm();await renderMethodologies();await refreshCalculationMethodologies()}
async function refreshCalculationMethodologies(){if(!db)return;let shift=$('#calcMetShift')?.value,group=$('#calcMetGroup')?.value,allM=await all('methodologies'),filtered=allM.filter(m=>sameShift(m.shift,shift)&&sameGroup(m.group,group)),sel=$('#calcMethodology');if(!sel)return;let old=sel.value;sel.innerHTML=filtered.length?filtered.map(m=>`<option value="${m.id}">${safe(m.name)}</option>`).join(''):'<option value="">Sin metodología</option>';if(filtered.some(m=>m.id===old))sel.value=old;await renderMethodologyAssignments()}
async function renderMethodologyAssignments(){let id=$('#calcMethodology').value,box=$('#methodologyAssignments');if(!id){box.className='list empty';box.textContent='Selecciona una metodología.';$('#methodologyResults').innerHTML='<div class="empty">Pulsa Calcular.</div>';return}let m=await req(store('methodologies').get(id)),acts=(await all('activities')).filter(a=>sameShift(a.shift,m.shift)&&sameGroup(a.group,m.group)).sort(activitySort);if(!acts.length){box.className='list empty';box.textContent='No hay actividades creadas para este grupo.';return}box.className='list';box.innerHTML=acts.map(a=>`<div class="assignment-row"><div><strong>${safe(a.name)} <span class="activity-mode ${a.evaluationMode||'delivery'}">${(a.evaluationMode||'delivery')==='numeric'?'Numérica 0–10':'Entrega 1/0'}</span></strong><small>${safe(a.week)} · ${safe(a.date)} · ${safe(a.type||'Actividad')}</small></div><select data-assignment="${a.id}"><option value="">Sin asignar</option>${m.criteria.map(c=>`<option value="${c.id}" ${m.assignments?.[a.id]===c.id?'selected':''}>${safe(c.name)} (${c.percent}%)</option>`).join('')}</select></div>`).join('')}
async function saveAssignments(){let id=$('#calcMethodology').value;if(!id)return alert('Selecciona una metodología.');let m=await req(store('methodologies').get(id)),assignments={};$$('[data-assignment]').forEach(s=>{if(s.value)assignments[s.dataset.assignment]=s.value});m.assignments=assignments;m.updated=new Date().toISOString();await put('methodologies',m);alert('Asignaciones guardadas.')}
async function calculateMethodology(){let id=$('#calcMethodology').value;if(!id)return alert('Selecciona una metodología.');let m=await req(store('methodologies').get(id)),total=m.criteria.reduce((s,c)=>s+Number(c.percent),0);if(Math.abs(total-100)>0.001)return alert('Los criterios de esta metodología no suman 100%. Edítala antes de calcular.');let acts=(await all('activities')).filter(a=>m.assignments?.[a.id]),assignedIds=new Set(acts.map(a=>a.id));if(!acts.length)return alert('Asigna al menos una actividad a un criterio.');let roster=(await students()).filter(s=>sameShift(s.shift,m.shift)&&sameGroup(s.group,m.group)),records=await all('activityRecords'),map=new Map(records.map(r=>[r.key,r]));let rows=roster.map(st=>{let criterionGrades={},pending=[];for(let c of m.criteria){let ca=acts.filter(a=>m.assignments[a.id]===c.id),values=[];for(let a of ca){let rec=map.get(`${a.id}|${st.id}`),mode=a.evaluationMode||'delivery';if(mode==='numeric'){if(typeof rec?.score==='number')values.push(rec.score);else pending.push(a.name)}else{if(rec?.status==='yes')values.push(10);else if(rec?.status==='no')values.push(0);else pending.push(a.name)}}criterionGrades[c.id]=ca.length&&values.length===ca.length?values.reduce((x,y)=>x+y,0)/values.length:null}let complete=pending.length===0&&m.criteria.every(c=>acts.some(a=>m.assignments[a.id]===c.id)&&criterionGrades[c.id]!==null),final=complete?m.criteria.reduce((sum,c)=>sum+(criterionGrades[c.id]*(c.percent/100)),0):null;return {student:st,criterionGrades,pending:[...new Set(pending)],final}});window._lastMethodologyCalculation={methodology:m,activities:acts,rows};renderMethodologyResults(window._lastMethodologyCalculation)}
function renderMethodologyResults(data){let {methodology:m,rows}=data,box=$('#methodologyResults');box.innerHTML=`<div class="methodology-summary"><strong>${safe(m.name)}</strong> · ${safe(m.subject||'Español')} · ${safe(m.month||'Mes sin definir')} · ${safe(m.shift)} · Grupo ${safe(m.group)}<br>${m.criteria.map(c=>`${safe(c.name)}: ${c.percent}%`).join(' · ')}</div><table class="matrix"><thead><tr><th>#</th><th class="name">Alumno</th>${m.criteria.map(c=>`<th>${safe(c.name)}<br>${c.percent}%</th>`).join('')}<th>Calificación final</th><th>Pendientes</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.student.number}</td><td class="name">${safe(r.student.name||r.student.id)}</td>${m.criteria.map(c=>`<td>${r.criterionGrades[c.id]===null?'—':r.criterionGrades[c.id].toFixed(1)}</td>`).join('')}<td class="${r.final===null?'grade-pending':'grade-good'}">${r.final===null?'Pendiente':r.final.toFixed(1)}</td><td>${r.pending.length?safe(r.pending.join(', ')):'—'}</td></tr>`).join('')}</tbody></table>`}
async function printMethodology(){let data=window._lastMethodologyCalculation;if(!data){await calculateMethodology();data=window._lastMethodologyCalculation}if(!data)return;if(!pdfReady())return alert('No se cargó el generador de PDF. Conéctate a internet, pulsa Actualizar app y vuelve a intentarlo.');let {methodology:m,rows}=data,{jsPDF}=window.jspdf,doc=new jsPDF({orientation:'landscape',unit:'mm',format:'a4'});pdfHeader(doc,'Cálculo general de metodología',(m.subject||'Español').toUpperCase());doc.setFontSize(10);doc.text(`${m.name} · Mes: ${m.month||'Sin definir'} · ${m.shift} · Grupo ${m.group}`,14,36);let head=[['#','Alumno',...m.criteria.map(c=>`${c.name} (${c.percent}%)`),'Final','Pendientes']],body=rows.map(r=>[String(r.student.number),r.student.name||r.student.id,...m.criteria.map(c=>r.criterionGrades[c.id]===null?'—':r.criterionGrades[c.id].toFixed(1)),r.final===null?'Pendiente':r.final.toFixed(1),r.pending.join(', ')||'—']);doc.autoTable({startY:41,head,body,styles:{fontSize:7,cellPadding:1.5,valign:'middle'},headStyles:{fillColor:[245,196,0],textColor:[33,27,18]},columnStyles:{1:{cellWidth:48}}});pdfFooter(doc);printContent('Cálculo de metodología',`<p><b>${safe(m.name)}</b> · ${safe(m.shift)} · Grupo ${safe(m.group)}</p>`+$('#methodologyResults').innerHTML,pdfBlob(doc),`metodologia_${m.group}_${m.name.replace(/[^a-z0-9]+/gi,'_')}.pdf`)}
...
async function snapshotData(){await ensureDB();let data={version:'5.5',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])data[n]=await all(n);return data}
...
async function restore(file){let d=JSON.parse(await file.text());for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])if(Array.isArray(d[n])){await clear(n);for(let x of d[n])await put(n,x)}await fillSelectors();refreshAttendance();await saveInternal(false);alert('Respaldo importado y guardado dentro de la app')}
...
async function buildTitularPdfBlobV55(){
  let sid=$('#titStudent').value,wid=$('#titSavedWeek').value;
  if(!sid||!wid){alert('Selecciona alumno y semana.');return null}
  let s=await req(store('students').get(sid)),w=await req(store('titularWeeks').get(wid)),rec=(await all('titularRecords')).filter(x=>x.weekId===wid&&x.studentId===sid),map=new Map(rec.map(x=>[x.subject,x]));
  if(!pdfReady())return alert('No se cargó el generador de PDF. Conéctate a internet, pulsa Actualizar app y vuelve a intentarlo.');
  let {jsPDF}=window.jspdf,doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});pdfHeader(doc,'Seguimiento académico 3.º A');doc.setFontSize(10);doc.setFont('helvetica','bold');doc.text('Titular: Profr. Jaime Armando Pérez Vázquez',14,33);doc.setFont('helvetica','normal');doc.text(`Alumno: ${s.name||s.id}`,14,39);doc.text(`Grupo: 3.º A    No. de lista: ${s.number}    Semana: ${w.label}`,14,45);doc.setFontSize(9);doc.setTextColor(20,80,30);doc.text('VERDE: al corriente.',14,54);doc.setTextColor(150,110,0);doc.text('AMARILLO: requiere acercarse con el docente para revisar pendientes.',14,60);doc.setTextColor(180,20,20);let lines=doc.splitTextToSize('ROJO - ATENCIÓN URGENTE: el alumno debe acercarse cuanto antes con el docente para revisar su situación y confirmar si existe posibilidad de ponerse al corriente. La recepción de trabajos atrasados depende de los criterios, fechas y autorización de cada maestro.',178);doc.text(lines,14,66);doc.setTextColor(80);let y=66+lines.length*4.2;doc.text('BLANCO: no se recibió información de la asignatura durante esta semana.',14,y);y+=8;const interpretation=st=>st==='green'?'Al corriente':st==='yellow'?'Requiere revisar pendientes con el docente':st==='red'?'ATENCIÓN URGENTE: acercarse con el docente':'Sin información esta semana';let body=subjects.map(sub=>{let st=map.get(sub)?.status||'white';return [sub,st==='green'?'Verde':st==='yellow'?'Amarillo':st==='red'?'Rojo':'Sin información',interpretation(st)]});doc.autoTable({startY:y,head:[['Asignatura','Estado','Interpretación']],body,styles:{fontSize:8,cellPadding:2},headStyles:{fillColor:[245,196,0],textColor:[33,27,18]},columnStyles:{0:{cellWidth:50},1:{cellWidth:32},2:{cellWidth:98}}});y=doc.lastAutoTable.finalY+8;doc.setTextColor(0);doc.setFont('helvetica','bold');doc.text('Observaciones generales:',14,y);doc.setFont('helvetica','normal');doc.text(doc.splitTextToSize(map.get('OBS')?.text||'Sin observaciones.',145),55,y);y+=14;doc.setFillColor(255,230,230);doc.rect(12,y-5,186,22,'F');doc.setTextColor(150,20,20);doc.setFont('helvetica','bold');doc.text(doc.splitTextToSize('IMPORTANTE: Este reporte es informativo. Cada docente determina si recibe actividades atrasadas, bajo qué condiciones y dentro de qué plazo. Un estado rojo no garantiza que los trabajos atrasados sean aceptados.',178),16,y);y+=27;doc.setFillColor(255,248,220);doc.rect(12,y-5,186,22,'F');doc.setTextColor(90,65,0);doc.text(doc.splitTextToSize('AVISO: Este reporte no sustituye la información oficial publicada en Systelar. Se recomienda revisar constantemente dicha plataforma para consultar avisos, calificaciones, pendientes y actualizaciones.',178),16,y);pdfFooter(doc);return {blob:pdfBlob(doc),fileName:`seguimiento_3A_${(s.name||s.id).replace(/[^a-z0-9]+/gi,'_')}_${w.label.replace(/[^a-z0-9]+/gi,'_')}.pdf`,student:s,week:w};
}
async function openTitularEmailDialog(){
