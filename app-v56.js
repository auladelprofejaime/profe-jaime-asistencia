...
async function snapshotData(){await ensureDB();let data={version:'5.6',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])data[n]=await all(n);return data}
...
async function buildTitularPdfBlobV55(){
  ...
  if(!pdfReady())return alert('No se cargó el generador de PDF. Conéctate a internet, pulsa Actualizar app y vuelve a intentarlo.');
  let {jsPDF}=window.jspdf,doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});pdfHeader(doc,'Seguimiento académico 3.º A');doc.setFontSize(10);doc.setFont('helvetica','bold');doc.text('Titular: Profr. Jaime Armando Pérez Vázquez',14,33);doc.setFont('helvetica','normal');doc.text(`Alumno: ${s.name||s.id}`,14,39);doc.text(`Grupo: 3.º A    No. de lista: ${s.number}    Semana: ${w.label}`,14,45);doc.setFontSize(9);doc.setTextColor(33,27,18);doc.setFillColor(22,163,74);doc.circle(16,52.8,2.2,'F');doc.text('VERDE: al corriente.',21,54);doc.setFillColor(212,168,0);doc.circle(16,58.8,2.2,'F');doc.text('AMARILLO: requiere acercarse con el docente para revisar pendientes.',21,60);doc.setFillColor(220,38,38);doc.circle(16,64.8,2.2,'F');doc.setTextColor(180,20,20);let lines=doc.splitTextToSize('ROJO - ATENCIÓN URGENTE: el alumno debe acercarse cuanto antes con el docente para revisar su situación y confirmar si existe posibilidad de ponerse al corriente. La recepción de trabajos atrasados depende de los criterios, fechas y autorización de cada maestro.',178);doc.text(lines,14,66);doc.setTextColor(80);let y=66+lines.length*4.2;doc.text('BLANCO: no se recibió información de la asignatura durante esta semana.',14,y);y+=8;const interpretation=st=>st==='green'?'Al corriente':st==='yellow'?'Requiere revisar pendientes con el docente':st==='red'?'ATENCIÓN URGENTE: acercarse con el docente':'Sin información esta semana';let body=subjects.map(sub=>{let st=map.get(sub)?.status||'white';return [sub,`__STATE_${st.toUpperCase()}__`,interpretation(st)]});doc.autoTable({startY:y,head:[['Asignatura','Estado','Interpretación']],body,styles:{fontSize:8,cellPadding:2,valign:'middle'},headStyles:{fillColor:[245,196,0],textColor:[33,27,18]},columnStyles:{0:{cellWidth:50},1:{cellWidth:32,halign:'center'},2:{cellWidth:98}},didParseCell:data=>{if(data.section==='body'&&typeof data.cell.raw==='string'&&data.cell.raw.startsWith('__STATE_'))data.cell.text=[''];},didDrawCell:data=>{if(data.section!=='body'||typeof data.cell.raw!=='string'||!data.cell.raw.startsWith('__STATE_'))return;let state=data.cell.raw.replace('__STATE_','').replace('__','').toLowerCase(),cx=data.cell.x+data.cell.width/2,cy=data.cell.y+data.cell.height/2,radius=2.8;if(state==='green')doc.setFillColor(22,163,74);else if(state==='yellow')doc.setFillColor(212,168,0);else if(state==='red')doc.setFillColor(220,38,38);else doc.setFillColor(255,255,255);doc.circle(cx,cy,radius,'F');if(state==='white'){doc.setDrawColor(145,145,145);doc.setLineWidth(.5);doc.circle(cx,cy,radius,'S')}}});y=doc.lastAutoTable.finalY+8;doc.setTextColor(0);doc.setFont('helvetica','bold');doc.text('Observaciones generales:',14,y);doc.setFont('helvetica','normal');doc.text(doc.splitTextToSize(map.get('OBS')?.text||'Sin observaciones.',145),55,y);y+=14;doc.setFillColor(255,230,230);doc.rect(12,y-5,186,22,'F');doc.setTextColor(150,20,20);doc.setFont('helvetica','bold');doc.text(doc.splitTextToSize('IMPORTANTE: Este reporte es informativo. Cada docente determina si recibe actividades atrasadas, bajo qué condiciones y dentro de qué plazo. Un estado rojo no garantiza que los trabajos atrasados sean aceptados.',178),16,y);y+=27;doc.setFillColor(255,248,220);doc.rect(12,y-5,186,22,'F');doc.setTextColor(90,65,0);doc.text(doc.splitTextToSize('AVISO: Este reporte no sustituye la información oficial publicada en Systelar. Se recomienda revisar constantemente dicha plataforma para consultar avisos, calificaciones, pendientes y actualizaciones.',178),16,y);pdfFooter(doc);return {blob:pdfBlob(doc),fileName:`seguimiento_3A_${(s.name||s.id).replace(/[^a-z0-9]+/gi,'_')}_${w.label.replace(/[^a-z0-9]+/gi,'_')}.pdf`,student:s,week:w};
}
async function openTitularEmailDialog(){
