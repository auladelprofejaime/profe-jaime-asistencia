...
async function importStudents(file){
 ...
 try{
  ...
  for(let o of rows){
   ...
   if(s.id&&s.number){let prior=await req(store('students').get(s.id));await put('students',{...(prior||{}),...s});count++}else skip++;
  }
...
async function saveStudent(e){
 ...
 if(old&&old!==s.id)await del('students',old);await put('students',s);e.target.reset();$('#editId').value='';$('#studentTitle').textContent='Agregar alumno';$('#cancelEdit').classList.add('hidden');await fillSelectors();await renderStudents();await refreshDossierSelector(s.id);
}
...
async function renderDossier(id){
 ...
 empty.classList.add('hidden');content.classList.remove('hidden');$('#dossierName').textContent=s.name||s.id;$('#dossierSchoolLine').textContent=`Grupo ${s.group} · No. de lista ${s.number} · ID ${s.id}`;let badge=$('#dossierShiftBadge');badge.textContent=s.shift||'Sin turno';badge.className='shift-badge '+(sameShift(s.shift,'Vespertino')?'vespertino':'');
 ...
 $('#dossierHistory').innerHTML=`<div class="history-item"><b>${att.length}</b><span>Asistencias registradas</span></div><div class="history-item"><b>${act.length}</b><span>Registros de actividades</span></div><div class="history-item"><b>${done}</b><span>Entregas o calificaciones</span></div><div class="history-item"><b>${tit.length}</b><span>Estados de seguimiento 3.º A</span></div>`;
...
async function snapshotData(){await ensureDB();let data={version:'6.1',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])data[n]=await all(n);return data}
