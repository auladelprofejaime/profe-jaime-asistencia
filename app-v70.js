...
const REQUIRED_STORES={students:'id',attendance:'key',activities:'id',activityRecords:'key',titularWeeks:'id',titularRecords:'key',settings:'key',internalBackups:'id',methodologies:'id',availability:'id',notices:'id',materials:'id',studyTopics:'id',studentMessages:'id',portalReports:'id'};
...
async function snapshotData(){await ensureDB();let data={version:'7.0',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies'])data[n]=await all(n);return data}
...
async function savePortalEntity(type,e){
 ...
 if(type==='material')await put('materials',{id:crypto.randomUUID(),group:norm($('#materialGroup').value),title:norm($('#materialTitle').value),type:$('#materialType').value,url:norm($('#materialUrl').value),created:new Date().toISOString(),active:true});
...
async function refreshPortalContent(){
 ...
 render('#materialList',await all('materials'),x=>`<div class="mini-item"><b>${safe(x.title)}</b><small>${safe(x.type)} · ${safe(x.group||'Todos')}</small></div>`);
