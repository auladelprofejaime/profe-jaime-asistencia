...
function remoteStudent(s){return {id:String(s.id),name:s.name||String(s.id),shift:s.shift||'',group_name:s.group||'',list_number:Number(s.number)||null,observations:s.observations||null,incidents:s.incidents||null,active:s.active!==false,inactive_at:s.inactiveAt||null,inactive_reason:s.inactiveReason||null}}
...
async function snapshotData(){await ensureDB();let data={version:'8.11.0',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies','availability','notices','materials','studyTopics','studentMessages','portalReports','portalAuth'])if(db.objectStoreNames.contains(n))data[n]=await all(n);if(Array.isArray(data.students))data.students=data.students.map(s=>{const x={...s};delete x.email;delete x.guardian;delete x.phone;delete x.birthdate;return x});if(Array.isArray(data.portalAuth))data.portalAuth=data.portalAuth.map(a=>{const x={...a};delete x.tempPinReveal;return x});if(Array.isArray(data.studentMessages))data.studentMessages=data.studentMessages.filter(chatMessageIsCurrentWeek);return data}
...
async function savePortalEntity(type,e){
 ...
 if(type==='notice'){
   ...
   await put('notices',item);pushed={event:'new_notice',title:item.title,group:item.group};
 }
 if(type==='material'){
...
async function checkTeacherMessageNotifications(initial=false){
 if(!supabaseReady)return;
 try{
   const cutoff=new Date(currentChatWeekStartISO()+'T00:00:00').toISOString();const remote=await ProfeSupabase.select('student_messages',`select=*&sent_at=gte.${encodeURIComponent(cutoff)}&order=sent_at.desc&limit=100`);
...
async function pullRemoteStudentMessages(){
 if(!supabaseReady)return;
 try{
   const cutoff=new Date(currentChatWeekStartISO()+'T00:00:00').toISOString();const remote=await ProfeSupabase.select('student_messages',`select=*&sent_at=gte.${encodeURIComponent(cutoff)}&order=sent_at.desc`);
