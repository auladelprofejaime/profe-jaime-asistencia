...
function meritPane(name){$$('.merit-pane').forEach(x=>x.classList.toggle('active',x.id==='merit-'+name));$$('.meritNav').forEach(x=>x.classList.toggle('active',x.dataset.meritPane===name));if(name==='ranking')loadMeritRanking();if(name==='movements')loadMeritMovements();if(name==='staff')loadMeritStaff();if(name==='weekly')meritWeeklyPreview();if(name==='config')loadMeritPeriods();}
...
async function loadMeritPeriods(){try{meritPeriodsCache=await meritRpc('teacher_merit_periods')||[];for(const id of ['meritRankingPeriod','meritMovementPeriod','meritWeeklyPeriod']){const el=$('#'+id);if(!el)continue;const old=el.value;el.innerHTML=meritPeriodsCache.map(p=>`<option value="${safe(p.id)}">${safe(p.label)} · ${safe(p.status)}</option>`).join('');if(meritPeriodsCache.some(p=>p.id===old))el.value=old;}const box=$('#meritPeriodsList');if(box)box.innerHTML=meritPeriodsCache.length?`<table><thead><tr><th>Periodo</th><th>Fechas</th><th>Estado</th><th>Portal</th></tr></thead><tbody>${meritPeriodsCache.map(p=>`<tr><td>${safe(p.label)}</td><td>${safe(p.starts_at)} → ${safe(p.ends_at)}</td><td>${safe(p.status)}</td><td>${safe(p.public_state)}</td></tr>`).join('')}</tbody></table>`:'<p class="hint">Aún no hay periodos.</p>';return meritPeriodsCache}catch(e){alert('No se pudieron cargar los periodos de Mérito: '+(e.message||e));return[]}}
...
async function registerAttendance(){
 ...
 if(sameShift(shift,'Matutino')){
   let message=st==='Retardo'
     ? `Se registró retardo de ${s.name||'el alumno'}.`
     : `Se registró la asistencia de ${s.name||'el alumno'} como presente.`;
   sendTeacherPushEvent('attendance_recorded',{
     student_id:String(s.id),
     student_name:s.name||'el alumno',
 ...
 for(const st of missing){
   ...
   if(sameShift(shift,'Matutino')){
     notifications.push(sendTeacherPushEvent('absence_recorded',{
       ...
       attendance_date:date,
       message:`Hoy se registró falta de ${st.name||'el alumno'}. No olviden pasar a Trabajo Social para obtener el justificante correspondiente.`
     }));
   }
 }
...
// ===== Mérito Gabino A. Palma · ajustes administrativos v8.15.1 =====
