...
async function notifyOpenedPointPeriods(periods){
  ...
  for(const p of periods||[]){
    ...
    try{
      ...
      const result=await sendTeacherPushEvent('point_period_open',{
      ...
      if(!result?.ok||sent<1){
        throw new Error(`send-push no confirmó entrega (sent=${sent}).`);
      }
...
async function sendTeacherPushEvent(event,body={}){
 ...
 try{
   const result=await ProfeSupabase.edge('send-push',{event,...body});
   return result;
 }catch(e){
