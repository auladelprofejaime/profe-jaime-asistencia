...
async function refreshHomeScheduleData(force=false){
  ...
  try{
   ...
   renderHomeScheduleWidget();
    ...
    console.warn('No se pudo actualizar panel de horario',e);
  }
}
function renderHomeScheduleWidget(){
  ...
  const countLabel=$('#homeScheduleCountLabel');
  ...
  if(!supabaseReady){
    ...
    timeEl.textContent='';
    countEl.textContent='--:--:--';
    countLabel.textContent='para terminar';
    nextEl.textContent='—';
    return;
  }
  ...
  const current=today.find(x=>{
    const start=hmToSeconds(x.start_time);
    const end=hmToSeconds(x.end_time);
  ...
  if(current){
    ...
    const remaining=hmToSeconds(current.end_time)-nowSec;

    currentEl.textContent=scheduleTitle(current);
    timeEl.textContent=fmtScheduleRange(current);
    countEl.textContent=fmtCountdown(remaining);
    countLabel.textContent='para terminar';
  ...
  if(next){
    const remaining=hmToSeconds(next.start_time)-nowSec;
    currentEl.textContent='Sin clase en este momento';
    timeEl.textContent='';
    countEl.textContent=fmtCountdown(remaining);
    countLabel.textContent='para la próxima clase';
...
async function startHomeScheduleWidget(){
  if(homeScheduleTimer){
    clearInterval(homeScheduleTimer);
    homeScheduleTimer=null;
  }
...
window.addEventListener('load',()=>{ setTimeout(()=>startHomeScheduleWidget(),400); });
document.addEventListener('visibilitychange',()=>{
  if(!document.hidden)startHomeScheduleWidget();
});
