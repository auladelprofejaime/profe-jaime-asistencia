...
async function printAttendanceRange(){
 ...
 const finishDate=new Date(to+'T12:00:00');
 const dates=[];
 ...
 const roster=(await students())
   .filter(x=>sameShift(x.shift,shift)&&sameGroup(x.group,group))
 ...
 doc.setFontSize(9);
 doc.text(`Turno: ${shift}    Grupo: ${group}    Periodo: ${from}${from===to?'':' al '+to}`,14,32);

 const head=['#','Alumno',...dates.map(d=>new Date(d+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit'}))];
 const body=roster.map(st=>[
   String(st.number??''),
   st.name||st.id,
   ...dates.map(d=>attendanceMark(byKey.get(`${d}|${st.id}`)))
 ]);
 ...
 doc.autoTable({
   ...
   head:[head],
   body,
   styles:{fontSize:dates.length>12?6.2:7.5,cellPadding:1.4,halign:'center'},
   headStyles:{fillColor:[245,196,0],textColor:[33,27,18]},
   columnStyles:{1:{halign:'left',cellWidth:landscape?72:70}},
 ...
 doc.setFontSize(8.5);
 doc.text('Leyenda: ✓ Asistencia    ✕ Inasistencia    ⌛ Retardo',14,y);
 doc.setFontSize(7.5);
 doc.text(`Generado: ${new Date().toLocaleString('es-MX')}`,14,y+5);
 pdfFooter(doc);
