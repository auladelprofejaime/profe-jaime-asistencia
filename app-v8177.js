...
function drawAttendanceSymbol(doc,mark,x,y,w,h){
 ...
 doc.setLineWidth(.45);
 if(mark==='P'){
   doc.line(cx-2.3,cy,cx-.7,cy+1.7);
   doc.line(cx-.7,cy+1.7,cx+2.7,cy-2);
 }else if(mark==='F'){
   doc.line(cx-2,cy-2,cx+2,cy+2);
   doc.line(cx+2,cy-2,cx-2,cy+2);
 }else if(mark==='R'){
   const left=cx-2.1,right=cx+2.1,top=cy-2.5,bottom=cy+2.5;
...
async function printAttendanceRange(){
 ...
 const drawPageHeader=(pageDates,pageIndex)=>{
   pdfHeader(doc,'Reporte de asistencia');
   doc.setFontSize(9);
   doc.setTextColor(40,40,40);
   const period=pageDates.length===1
     ? new Date(pageDates[0]+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'long',year:'numeric'})
     : `${new Date(pageDates[0]+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit',year:'numeric'})} al ${new Date(pageDates.at(-1)+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit',year:'numeric'})}`;
   doc.text(`Turno: ${shift}   Grupo: ${group}`,14,31);
   doc.text(`Periodo: ${period}`,14,36);
   if(chunks.length>1)doc.text(`Bloque ${pageIndex+1} de ${chunks.length}`,196,36,{align:'right'});
 };

 chunks.forEach((pageDates,pageIndex)=>{
   if(pageIndex>0)doc.addPage();
   drawPageHeader(pageDates,pageIndex);
   ...
   const pageWidth=210;
   const margins=14;
   const usable=pageWidth-margins*2;
   const noWidth=10;
   const studentWidth=pageDates.length===1?112:76;
   ...
   doc.autoTable({
     ...
     body,
     theme:'grid',
     margin:{left:margins,right:margins,top:15,bottom:24},
     styles:{
       fontSize:8.2,
       cellPadding:1.8,
       valign:'middle',
       lineColor:[205,190,150],
       lineWidth:.15,
       overflow:'linebreak'
     },
     headStyles:{
       fillColor:[248,200,10],
       textColor:[40,32,20],
       fontStyle:'bold',
       halign:'center',
       fontSize:8
     ...
     alternateRowStyles:{fillColor:[252,249,239]},
     columnStyles,
     didParseCell(data){
     ...
     didDrawPage(){
       ...
       doc.text('Leyenda:',14,ph-14);
       drawAttendanceSymbol(doc,'P',31,ph-19,8,8);
       doc.text('Asistencia',40,ph-14);
       drawAttendanceSymbol(doc,'F',68,ph-19,8,8);
       doc.text('Inasistencia',77,ph-14);
       drawAttendanceSymbol(doc,'R',110,ph-19,8,8);
       doc.text('Retardo',119,ph-14);
       doc.text(`Generado: ${new Date().toLocaleString('es-MX')}`,196,ph-14,{align:'right'});
     }
   });
 });

 pdfFooter(doc);
 const html=`<p><b>Turno:</b> ${safe(shift)} · <b>Grupo:</b> ${safe(group)}</p><p><b>Periodo:</b> ${safe(from)}${from===to?'':' al '+safe(to)}</p><p><b>Leyenda:</b> ✓ Asistencia · ✕ Inasistencia · ⌛ Retardo</p>`;
