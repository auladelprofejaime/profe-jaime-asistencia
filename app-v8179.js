...
function attendanceRosterMatch(student,shift,group){
  return sameShift(student.shift,shift) && sameGroup(student.group,group) && student.active!==false;
}
function attendanceId(value){ return norm(String(value??'')); }
function attendanceRecordMap(records){
  const m=new Map();
  for(const r of records||[])m.set(attendanceId(r.studentId),r);
  return m;
}
function attendanceStatusLabel(status){
  if(status==='Falta')return 'Inasistencia';
  if(status==='Retardo')return 'Retardo';
  if(status==='Presente')return 'Asistencia';
  return 'Sin registrar';
}
async function updateAttendanceStatusForStudent(studentId,newStatus){
  const shift=$('#attShift').value,group=$('#attGroup').value,date=$('#attDate').value;
  const roster=(await students()).filter(st=>attendanceRosterMatch(st,shift,group));
  const st=roster.find(x=>attendanceId(x.id)===attendanceId(studentId));
  if(!st)return alert('No se encontró al alumno en el grupo seleccionado.');

  const key=`${date}|${st.id}`;
  const old=await req(store('attendance').get(key));
  const timestamp=new Date().toISOString();
  const rec={
    key,date,studentId:st.id,name:st.name,shift:st.shift,group:String(st.group),
    number:st.number,status:newStatus,timestamp
  };
  await put('attendance',rec);

  if(sameShift(shift,'Matutino') && newStatus!=='Falta'){
    ...
    const isLate=newStatus==='Retardo';
    sendTeacherPushEvent('attendance_recorded',{
      title:isLate?'Retardo registrado':'Asistencia registrada',
      student_id:String(st.id),
      student_name:st.name||'Alumno',
      attendance_date:date,
      attendance_time:timeLabel,
      attendance_status:newStatus,
      message:`${st.name||'Alumno'} · Fecha: ${dateLabel} · Hora: ${timeLabel}`
    }).catch(()=>{});
  }

  const oldLabel=attendanceStatusLabel(old?.status);
  const newLabel=attendanceStatusLabel(newStatus);
  status('att','success',`${newLabel.toUpperCase()} REGISTRADA`,`${st.name||st.id} · ${oldLabel} → ${newLabel}`);
  await refreshAttendance();
}
async function registerAttendanceManual(studentId,statusValue){
  $('#attScan').value=studentId;
  $('#attMode').value=statusValue==='Retardo'?'retardo':'presente';
...
async function refreshAttendance(){
 ...
 const roster=(await students())
   .filter(x=>attendanceRosterMatch(x,shift,group))
   .sort((x,y)=>(Number(x.number)||999)-(Number(y.number)||999)||String(x.name||'').localeCompare(String(y.name||''),'es'));
 const dayRecords=(await all('attendance'))
   .filter(x=>x.date===date&&sameShift(x.shift,shift)&&sameGroup(x.group,group));
 const recMap=attendanceRecordMap(dayRecords);

 $('#attPresent').textContent=dayRecords.length;
 $('#attTotal').textContent=roster.length;
 $('#attMissing').textContent=roster.filter(st=>!recMap.has(attendanceId(st.id))).length;

 const box=$('#attList');
 if(!roster.length){
   box.className='list empty';
   box.textContent='No hay alumnos cargados en este turno y grupo.';
   return;
 }
 box.className='list';
 box.innerHTML=roster.map(st=>{
   const rec=recMap.get(attendanceId(st.id));
   const current=attendanceStatusLabel(rec?.status);
   const time=rec?.timestamp?new Date(rec.timestamp).toLocaleTimeString('es-MX',{hour:'numeric',minute:'2-digit'}):'';
   const statusClass=rec?.status==='Falta'?'attendance-absent':rec?.status==='Retardo'?'attendance-late':rec?.status==='Presente'?'attendance-present':'attendance-pending';

   let actions='';
   if(!rec){
     actions=`<button class="secondary" data-att-present="${safe(st.id)}">Asistencia</button>
              <button class="secondary" data-att-late="${safe(st.id)}">Retardo</button>`;
   }else if(rec.status==='Falta'){
     actions=`<button class="secondary" data-att-late-edit="${safe(st.id)}">Cambiar a retardo</button>
              <button class="secondary" data-att-present-edit="${safe(st.id)}">Cambiar a asistencia</button>`;
   }else if(rec.status==='Retardo'){
     actions=`<button class="secondary" data-att-present-edit="${safe(st.id)}">Cambiar a asistencia</button>`;
   }else if(rec.status==='Presente'){
     actions=`<button class="secondary" data-att-late-edit="${safe(st.id)}">Cambiar a retardo</button>`;
   }
   if(rec)actions+=`<button class="del" data-ad="${safe(rec.key)}">Eliminar</button>`;

   return `<div class="row attendance-row ${statusClass}">
     <div>
       <strong>${safe(st.name||st.id)}</strong>
       <small>Lista ${safe(st.number)} · <b>${safe(current)}</b>${time?' · '+safe(time):''}</small>
     </div>
     <div class="rowactions">${actions}</div>
   </div>`;
 }).join('');

 $$('[data-att-present]').forEach(btn=>btn.onclick=()=>registerAttendanceManual(btn.dataset.attPresent,'Presente'));
 $$('[data-att-late]').forEach(btn=>btn.onclick=()=>registerAttendanceManual(btn.dataset.attLate,'Retardo'));
 $$('[data-att-present-edit]').forEach(btn=>btn.onclick=()=>updateAttendanceStatusForStudent(btn.dataset.attPresentEdit,'Presente'));
 $$('[data-att-late-edit]').forEach(btn=>btn.onclick=()=>updateAttendanceStatusForStudent(btn.dataset.attLateEdit,'Retardo'));
 $$('[data-ad]').forEach(btn=>btn.onclick=async()=>{
   if(confirm('¿Eliminar este registro?')){
     await del('attendance',btn.dataset.ad);
     await refreshAttendance();
   }
 });
}
...
async function registerAttendance(){
 ...
 if(sameShift(shift,'Matutino')){
   ...
   const message=`${s.name||'Alumno'} · Fecha: ${dateLabel} · Hora: ${timeLabel}`;
   sendTeacherPushEvent('attendance_recorded',{
     title:isLate?'Retardo registrado':'Asistencia registrada',
...
async function showMissing(){
 ...
 const roster=(await students())
   .filter(x=>attendanceRosterMatch(x,shift,group))
   .sort((x,y)=>(Number(x.number)||999)-(Number(y.number)||999));
 const dayRecords=(await all('attendance'))
   .filter(x=>x.date===date&&sameShift(x.shift,shift)&&sameGroup(x.group,group));
 const recMap=attendanceRecordMap(dayRecords);
 const missing=roster.filter(st=>!recMap.has(attendanceId(st.id)));

 if(!roster.length){
   showDialog('Faltantes','<p>No hay alumnos cargados en el turno y grupo seleccionados.</p>');
   return;
 }
 showDialog(
   `Faltantes · ${missing.length}`,
   missing.length
     ? missing.map(x=>`<div class="row"><div><strong>${safe(x.name||'Sin nombre')}</strong><small>ID ${safe(x.id)} · Lista ${safe(x.number)} · Sin registrar</small></div></div>`).join('')
     : '<p>No hay alumnos pendientes de registrar.</p>'
 );
}

async function finalizeAttendance(){
 const shift=$('#attShift').value,group=$('#attGroup').value,date=$('#attDate').value;
 const roster=(await students()).filter(x=>attendanceRosterMatch(x,shift,group));
 const dayRecords=(await all('attendance'))
   .filter(x=>x.date===date&&sameShift(x.shift,shift)&&sameGroup(x.group,group));
 const recMap=attendanceRecordMap(dayRecords);
 const missing=roster.filter(st=>!recMap.has(attendanceId(st.id)));

 if(!roster.length){alert('No hay alumnos cargados en el turno y grupo seleccionados.');return}
 if(!missing.length){alert('No hay alumnos pendientes. La lista ya está completa.');return}
 if(!confirm(`Se registrará INASISTENCIA a ${missing.length} alumno${missing.length===1?'':'s'}.\n\nDespués podrás cambiar una inasistencia a RETARDO si el alumno llega tarde.\n\n¿Finalizar la lista?`))return;
 ...
 for(const st of missing){
   ...
   if(sameShift(shift,'Matutino')){
     ...
     notifications.push(sendTeacherPushEvent('absence_recorded',{
       ...
       attendance_time:when.toLocaleTimeString('es-MX',{hour:'numeric',minute:'2-digit'}),
       message:`${st.name||'Alumno'} · Fecha: ${when.toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit',year:'numeric'})} · Hora: ${when.toLocaleTimeString('es-MX',{hour:'numeric',minute:'2-digit'})} · No olvide justificar la inasistencia con Trabajo Social.`
     }));
   }
 }
 if(notifications.length)await Promise.allSettled(notifications);
 await refreshAttendance();
 alert(`Lista finalizada. Se registraron ${missing.length} inasistencia${missing.length===1?'':'s'}.\n\nSi alguno llega después, usa “Cambiar a retardo” en su registro.`);
}
...
async function printAttendance(){
 ...
 $('#attReportFrom').value=date;
 $('#attReportTo').value=date;
 return printAttendanceRange();
}
function attendanceMark(rec){
 if(!rec)return '';
 if(rec.status==='Falta')return 'F';
 if(rec.status==='Retardo')return 'R';
 return 'P';
}
function drawAttendanceSymbol(doc,mark,x,y,w,h){
 const cx=x+w/2,cy=y+h/2;
 const r=Math.min(w,h)*.27;

 if(mark==='P'){
   doc.setFillColor(232,247,236);
   doc.setDrawColor(51,139,72);
   doc.circle(cx,cy,r,'FD');
   doc.setDrawColor(40,125,61);
   doc.setLineWidth(.7);
   doc.line(cx-r*.48,cy,cx-r*.12,cy+r*.38);
   doc.line(cx-r*.12,cy+r*.38,cx+r*.58,cy-r*.48);
 }else if(mark==='F'){
   doc.setFillColor(253,235,237);
   doc.setDrawColor(190,35,48);
   doc.circle(cx,cy,r,'FD');
   doc.setDrawColor(181,31,40);
   doc.setLineWidth(.65);
   doc.line(cx-r*.42,cy-r*.42,cx+r*.42,cy+r*.42);
   doc.line(cx+r*.42,cy-r*.42,cx-r*.42,cy+r*.42);
 }else if(mark==='R'){
   doc.setFillColor(255,247,219);
   doc.setDrawColor(207,151,0);
   doc.circle(cx,cy,r,'FD');

   const hw=r*.42,hh=r*.52;
   doc.setDrawColor(183,126,0);
   doc.setLineWidth(.45);
   doc.line(cx-hw,cy-hh,cx+hw,cy-hh);
   doc.line(cx-hw,cy+hh,cx+hw,cy+hh);
   doc.line(cx-hw,cy-hh,cx+hw,cy+hh);
   doc.line(cx+hw,cy-hh,cx-hw,cy+hh);

   doc.setFillColor(183,126,0);
   doc.triangle(cx-hw*.38,cy-hh*.28,cx+hw*.38,cy-hh*.28,cx,cy,'F');
   doc.triangle(cx-hw*.42,cy+hh*.35,cx+hw*.42,cy+hh*.35,cx,cy+hh*.05,'F');
 }
}
function attendanceStatusCounts(roster,pageDates,byKey){
 let present=0,absent=0,late=0,pending=0;
 for(const st of roster){
   for(const d of pageDates){
     const rec=byKey.get(`${d}|${attendanceId(st.id)}`);
     if(!rec){pending++;continue}
     if(rec.status==='Falta')absent++;
     else if(rec.status==='Retardo')late++;
     else present++;
   }
 }
 return {present,absent,late,pending};
}
function drawReportStat(doc,x,y,w,label,value,type){
 let bg=[248,248,248],border=[215,215,215],text=[45,45,45];
 if(type==='present'){bg=[235,248,238];border=[104,175,119];text=[43,126,61]}
 if(type==='absent'){bg=[254,238,239];border=[215,110,119];text=[181,31,40]}
 if(type==='late'){bg=[255,248,225];border=[224,183,80];text=[166,114,0]}
 if(type==='total'){bg=[245,245,245];border=[190,190,190];text=[45,45,45]}
 doc.setFillColor(...bg);
 doc.setDrawColor(...border);
 doc.roundedRect(x,y,w,15,2.5,2.5,'FD');
 doc.setFont('helvetica','bold');
 doc.setFontSize(13);
 doc.setTextColor(...text);
 doc.text(String(value),x+w/2,y+6.5,{align:'center'});
 doc.setFont('helvetica','normal');
 doc.setFontSize(6.8);
 doc.setTextColor(80,80,80);
 doc.text(label,x+w/2,y+11.8,{align:'center'});
}

function drawAttendanceReportHeader(doc,shift,group,periodText,pageIndex,totalPages,counts){
  const logoData='data:image/png;base64,';
  ...
  doc.setFillColor(249,198,0);
  doc.rect(0,0,210,5,'F');

  // Logo
  try{
    doc.addImage(logoData,'PNG',14,9,24,24,undefined,'FAST');
  }catch(e){}
  ...
  doc.setFont('helvetica','bold');
  doc.setFontSize(18);
  doc.text('Reporte de asistencia',44,18);

  doc.setFont('helvetica','normal');
  doc.setFontSize(8.2);
  doc.setTextColor(105,105,105);
  doc.text('El Aula del Profe Jaime · App Docente',44,24);

  // Línea roja inferior al encabezado
  doc.setDrawColor(184,31,43);
  doc.setLineWidth(1);
  ...
  doc.roundedRect(14,41,182,30,3,3,'FD');

  doc.setTextColor(52,52,52);
  doc.setFont('helvetica','bold');
  doc.setFontSize(8.2);

  doc.text('Profesor',20,49);
  doc.text('Grupo',20,60);
  doc.text('Fecha',104,49);
  doc.text('Turno',104,60);

  doc.setFont('helvetica','normal');
  doc.setFontSize(8.5);
  doc.text('Jaime Armando Perez Vazquez',20,54);
  doc.text(String(group),20,65);
  doc.text(periodText,104,54,{maxWidth:84});
  doc.text(String(shift),104,65);

  if(totalPages>1){
    doc.setFontSize(6.8);
    doc.setTextColor(120,120,120);
    doc.text(`Pagina ${pageIndex+1} de ${totalPages}`,196,74,{align:'right'});
  }

  // Resumen
  const gap=3, sw=(182-gap*3)/4;
  let x=14;
  drawReportStat(doc,x,77,sw,'Registros',counts.total,'total'); x+=sw+gap;
  drawReportStat(doc,x,77,sw,'Asistencias',counts.present,'present'); x+=sw+gap;
  drawReportStat(doc,x,77,sw,'Inasistencias',counts.absent,'absent'); x+=sw+gap;
  drawReportStat(doc,x,77,sw,'Retardos',counts.late,'late');
}
...
async function printAttendanceRange(){
 ...
 const startDate=new Date(from+'T12:00:00'),finishDate=new Date(to+'T12:00:00');
 const dates=[];
 for(let d=new Date(startDate);d<=finishDate;d.setDate(d.getDate()+1)){
   dates.push(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`);
 }
 if(dates.length>31)return alert('Genera un rango máximo de 31 días.');

 const roster=(await students())
   .filter(x=>attendanceRosterMatch(x,shift,group))
   .sort((a,b)=>(Number(a.number)||999)-(Number(b.number)||999)||String(a.name||'').localeCompare(String(b.name||''),'es'));

 if(!roster.length)return alert('No hay alumnos cargados en el turno y grupo seleccionados.');

 const allAtt=(await all('attendance'))
   .filter(x=>sameShift(x.shift,shift)&&sameGroup(x.group,group)&&x.date>=from&&x.date<=to);
 const byKey=new Map(allAtt.map(x=>[`${x.date}|${attendanceId(x.studentId)}`,x]));
 ...
 // 1 fecha luce como reporte individual; rangos se dividen en bloques de 7 fechas.
 const perPage=dates.length===1?1:7;
 const chunks=[];
 for(let i=0;i<dates.length;i+=perPage)chunks.push(dates.slice(i,i+perPage));

 chunks.forEach((pageDates,pageIndex)=>{
   if(pageIndex>0)doc.addPage();

   const periodText=pageDates.length===1
     ? new Date(pageDates[0]+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'long',year:'numeric'})
     : `${new Date(pageDates[0]+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit',year:'numeric'})} - ${new Date(pageDates.at(-1)+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit',year:'numeric'})}`;

   const counts=attendanceStatusCounts(roster,pageDates,byKey);
   counts.total=roster.length*pageDates.length;

   drawAttendanceReportHeader(doc,shift,group,periodText,pageIndex,chunks.length,counts);

   const head=['#','Alumno',...pageDates.map(d=>new Date(d+'T12:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit'}))];
   const body=roster.map(st=>[
     String(st.number??''),
     st.name||st.id,
     ...pageDates.map(d=>attendanceMark(byKey.get(`${d}|${attendanceId(st.id)}`)))
   ]);

   const margins=14;
   const usable=182;
   const noWidth=10;
   const studentWidth=pageDates.length===1?116:78;
   const dateWidth=(usable-noWidth-studentWidth)/pageDates.length;
   const columnStyles={
     0:{cellWidth:noWidth,halign:'center',fontStyle:'bold'},
     1:{cellWidth:studentWidth,halign:'left'}
   };
   for(let i=0;i<pageDates.length;i++)columnStyles[i+2]={cellWidth:dateWidth,halign:'center'};
   ...
   doc.autoTable({
     ...
     body,
     theme:'grid',
     margin:{left:margins,right:margins,top:14,bottom:27},
     styles:{
       fontSize:8.3,
       cellPadding:2.1,
       valign:'middle',
       lineColor:[221,210,177],
       lineWidth:.18,
       overflow:'linebreak',
       textColor:[50,50,50]
     },
     ...
     didDrawCell(data){
       if(data.section==='body' && data.column.index>=2){
         ...
         const mark=rowData[data.column.index];
         drawAttendanceSymbol(doc,mark,data.cell.x,data.cell.y,data.cell.width,data.cell.height);
       }
     }
   });
   ...
   doc.setDrawColor(226,216,188);
   doc.roundedRect(14,ph-23,111,12,2.5,2.5,'FD');

   doc.setFont('helvetica','bold');
   doc.setFontSize(7.3);
   doc.setTextColor(70,70,70);
   doc.text('Leyenda',19,ph-16);

   drawAttendanceSymbol(doc,'P',39,ph-21,8,8);
   doc.setFont('helvetica','normal');
   doc.setFontSize(6.7);
   doc.text('Asistencia',48,ph-16);

   drawAttendanceSymbol(doc,'F',71,ph-21,8,8);
   doc.text('Inasistencia',80,ph-16);

   drawAttendanceSymbol(doc,'R',109,ph-21,8,8);
   doc.text('Retardo',118,ph-16);

   // footer right
   doc.setFontSize(6.7);
   doc.setTextColor(110,110,110);
   doc.text('El Aula del Profe Jaime · App Docente',196,ph-18,{align:'right'});
   doc.text(new Date().toLocaleString('es-MX'),196,ph-14,{align:'right'});

   doc.setDrawColor(190,30,43);
   doc.setLineWidth(.8);
   doc.line(14,ph-8,196,ph-8);
 });

 const html=`<p><b>Turno:</b> ${safe(shift)} · <b>Grupo:</b> ${safe(group)}</p><p><b>Periodo:</b> ${safe(from)}${from===to?'':' al '+safe(to)}</p><p>Reporte generado desde El Aula del Profe Jaime.</p>`;
 printContent('Reporte de asistencia',html,pdfBlob(doc),`asistencia_${shift}_${group}_${from}_${to}.pdf`);
}


let homeScheduleTimer=null;
let homeScheduleLastFetch=0;

function scheduleDayNumberLocal(date=new Date()){
  const d=date.getDay();
  return d===0?7:d;
}
function hmToSeconds(value){
  const parts=String(value||'00:00').slice(0,8).split(':').map(Number);
  return (parts[0]||0)*3600+(parts[1]||0)*60+(parts[2]||0);
}
function currentSecondsLocal(date=new Date()){
...
function fmtScheduleRange(row){
  return row?`${String(row.start_time||'').slice(0,5)}–${String(row.end_time||'').slice(0,5)}`:'';
}
function scheduleTitle(row){
  if(!row)return '—';
  const subject=String(row.subject||'Actividad').trim();
  const group=String(row.group_name||'').trim();
  return group?`${subject} · ${group}`:subject;
}
async function refreshHomeScheduleData(force=false){
  if(!supabaseReady)return;
  const now=Date.now();
  if(!force && now-homeScheduleLastFetch<30000)return;
  try{
    scheduleCache=await ProfeSupabase.rpc('teacher_schedule_list',{})||[];
   homeScheduleLastFetch=Date.now();
   renderHomeScheduleWidget();
    homeScheduleLastFetch=now;
  }catch(e){console.warn('Panel horario:',e)}
}
function renderHomeScheduleWidget(){
  const box=$('#homeScheduleWidget');
  if(!box)return;
  const currentEl=$('#homeScheduleCurrent');
  const timeEl=$('#homeScheduleTime');
  const countEl=$('#homeScheduleCountdown');
  const labelEl=$('#homeScheduleCountLabel');
  const nextEl=$('#homeScheduleNext');

  if(!supabaseReady){
    currentEl.textContent='Conecta Supabase';
    timeEl.textContent='';
    countEl.textContent='--:--:--';
    labelEl.textContent='para terminar';
    nextEl.textContent='—';
    return;
  }

  const now=new Date();
  const dow=scheduleDayNumberLocal(now);
  const nowSec=currentSecondsLocal(now);
  const today=(scheduleCache||[])
    .filter(x=>Number(x.day_of_week)===dow && x.enabled!==false)
    .sort((a,b)=>hmToSeconds(a.start_time)-hmToSeconds(b.start_time));

  if(!today.length){
    currentEl.textContent='Sin clases programadas hoy';
    timeEl.textContent='';
    countEl.textContent='--:--:--';
    labelEl.textContent='';
    nextEl.textContent='—';
  ...
  const current=today.find(x=>nowSec>=hmToSeconds(x.start_time)&&nowSec<hmToSeconds(x.end_time));
  if(current){
    const idx=today.findIndex(x=>String(x.id)===String(current.id));
    const next=today.slice(idx+1).find(x=>hmToSeconds(x.start_time)>=hmToSeconds(current.end_time))||null;
    currentEl.textContent=scheduleTitle(current);
    timeEl.textContent=fmtScheduleRange(current);
    countEl.textContent=fmtCountdown(hmToSeconds(current.end_time)-nowSec);
    labelEl.textContent='para terminar';
    nextEl.textContent=next?`${scheduleTitle(next)} · ${fmtScheduleRange(next)}`:'Jornada terminada';
    return;
  }

  const next=today.find(x=>hmToSeconds(x.start_time)>nowSec);
  if(next){
    currentEl.textContent='Sin clase en este momento';
    timeEl.textContent='';
    countEl.textContent=fmtCountdown(hmToSeconds(next.start_time)-nowSec);
    labelEl.textContent='para la próxima clase';
    nextEl.textContent=`${scheduleTitle(next)} · ${fmtScheduleRange(next)}`;
  }else{
    currentEl.textContent='Jornada terminada';
    timeEl.textContent='';
    countEl.textContent='00:00:00';
    labelEl.textContent='';
    nextEl.textContent='No hay más clases hoy';
  }
}
async function startHomeScheduleWidget(){
  if(homeScheduleTimer)clearInterval(homeScheduleTimer);
  await refreshHomeScheduleData(true);
  renderHomeScheduleWidget();
  let tick=0;
  homeScheduleTimer=setInterval(async()=>{
    tick++;
    if(tick%30===0)await refreshHomeScheduleData(false);
    renderHomeScheduleWidget();
  },1000);
}
...
async function refreshSchedule(){
 ...
 if(!supabaseReady){
   box.className='list empty';
   box.textContent='Conéctate a Supabase para cargar el horario.';
   return;
 }
 try{
   scheduleCache=await ProfeSupabase.rpc('teacher_schedule_list',{})||[];
   if(!scheduleCache.length){
     box.className='list empty';
     box.textContent='Aún no has cargado clases.';
     return;
   }
   box.className='list';
   box.innerHTML=scheduleCache
     .sort((a,b)=>Number(a.day_of_week)-Number(b.day_of_week)||String(a.start_time).localeCompare(String(b.start_time)))
     .map(x=>`<div class="row">
       <div>
         <strong>${safe(SCHEDULE_DAYS[x.day_of_week]||x.day_of_week)} · ${safe(String(x.start_time||'').slice(0,5))}–${safe(String(x.end_time||'').slice(0,5))}</strong>
         <small>${safe(x.shift)} · ${safe(x.group_name)} · ${safe(x.subject)} · aviso ${safe(x.reminder_minutes)} min antes de terminar${x.enabled?'':' · DESACTIVADO'}</small>
       </div>
       <div class="rowactions">
         <button data-schedule-edit="${safe(x.id)}" class="secondary">Editar</button>
         <button data-schedule-del="${safe(x.id)}" class="del">Eliminar</button>
       </div>
     </div>`).join('');
   ...
   box.className='list empty';
   box.textContent='No se pudo cargar el horario: '+(e.message||e);
 }
}

function resetScheduleForm(){
 const form=$('#scheduleForm'); if(form)form.reset();
 $('#scheduleId').value='';
 $('#scheduleSubject').value='Español';
 $('#scheduleReminder').value='5';
 $('#scheduleEnabled').checked=true;
 $('#scheduleCancelEdit').classList.add('hidden');
}

function editScheduleItem(id){
 const x=scheduleCache.find(r=>String(r.id)===String(id));
 if(!x)return;
 $('#scheduleId').value=x.id;
 $('#scheduleDay').value=String(x.day_of_week);
 $('#scheduleShift').value=x.shift;
 $('#scheduleStart').value=String(x.start_time).slice(0,5);
 $('#scheduleEnd').value=String(x.end_time).slice(0,5);
 $('#scheduleGroup').value=x.group_name;
 $('#scheduleSubject').value=x.subject;
 $('#scheduleReminder').value=String(x.reminder_minutes||5);
 $('#scheduleEnabled').checked=x.enabled!==false;
...
async function saveScheduleItem(e){
 e.preventDefault();
 if(!supabaseReady)return alert('Conéctate a Supabase antes de guardar el horario.');
 try{
   await ProfeSupabase.rpc('teacher_schedule_save',{
     p_id:$('#scheduleId').value||null,
     p_day_of_week:Number($('#scheduleDay').value),
     p_start_time:$('#scheduleStart').value,
     p_end_time:$('#scheduleEnd').value,
     p_shift:$('#scheduleShift').value,
     p_group_name:$('#scheduleGroup').value.trim(),
     p_subject:$('#scheduleSubject').value.trim(),
     p_reminder_minutes:Number($('#scheduleReminder').value),
     p_enabled:$('#scheduleEnabled').checked
   });
   resetScheduleForm();
   await refreshSchedule();
   alert('Clase guardada en Mi horario.');
 }catch(e){
   alert('No se pudo guardar la clase: '+(e.message||e));
 }
}
...
async function deleteScheduleItem(id){
 ...
 try{
   await ProfeSupabase.rpc('teacher_schedule_delete',{p_id:id});
   await refreshSchedule();
...
async function pauseScheduleToday(){
 try{
   const r=await ProfeSupabase.rpc('teacher_schedule_pause_today',{});
   alert(r?.paused?'Avisos de hoy pausados.':'Los avisos de hoy volvieron a activarse.');
   $('#schedulePauseToday').textContent=r?.paused?'Reactivar avisos de hoy':'Pausar avisos de hoy';
 }catch(e){alert(e.message||e)}
}

async function enableSchedulePush(){
 try{
   if(!('Notification' in window)||!('serviceWorker' in navigator))return alert('Este dispositivo no admite notificaciones web.');
   if(Notification.permission!=='granted'){
     const p=await Notification.requestPermission();
     if(p!=='granted')return alert('No se concedió permiso para notificaciones.');
   }
   ...
   const j=sub.toJSON();
   await ProfeSupabase.rpc('register_schedule_push_subscription',{
   ...
   alert('No se pudieron activar los avisos: '+(e.message||e));
 }
}
...
async function openTitularEmailDialog(){
  ...
  $('#openMailComposer').onclick=()=>{
    ...
    if(!to)return alert('Escribe el correo institucional para este envío. No se guardará en el expediente.');
...
async function importStudents(file){
 ...
 try{
  ...
  for(let o of rows){
   ...
   let legacyBirth=norm(pick(o,['Fecha de nacimiento','Nacimiento']));
   let bm=legacyBirth.match(/^\d{4}-(\d{1,2})-(\d{1,2})/);
   let s={id,shift:norm(pick(o,['Turno']))||'Vespertino',group:norm(pick(o,['Grupo']))||'3A',number:num,
   name:norm(pick(o,['Nombre y primer apellido','Nombre del alumno','Nombre completo del alumno','Nombre completo','Nombre','Alumno'])),
   observations:norm(pick(o,['Observaciones','Observación','Observacion'])),incidents:norm(pick(o,['Incidencias','Incidencia'])),
   birthDay:Number(pick(o,['Cumpleaños día','Cumpleanos dia','Día cumpleaños','Dia cumpleanos'])||0)||(bm?Number(bm[2]):0),
   birthMonth:Number(pick(o,['Cumpleaños mes','Cumpleanos mes','Mes cumpleaños','Mes cumpleanos'])||0)||(bm?Number(bm[1]):0)};
...
async function saveStudent(e){
 ...
 let s={...(prior||{}),active:prior?.active===false?false:true,id:norm($('#studentId').value),shift:$('#studentShift').value,group:norm($('#studentGroup').value),number:Number($('#studentNumber').value),name:norm($('#studentName').value),birthDay:Number($('#studentBirthDay').value||0),birthMonth:Number($('#studentBirthMonth').value||0),observations:norm($('#studentObservations').value),incidents:norm($('#studentIncidents').value)};delete s.email;delete s.guardian;delete s.phone;delete s.birthdate;
...
function fillBirthdayDayOptions(){const s=$('#studentBirthDay');if(!s)return;s.innerHTML='<option value="">Sin registrar</option>'+Array.from({length:31},(_,i)=>`<option value="${i+1}">${i+1}</option>`).join('')}function fillStudentForm(x){$('#editId').value=x.id;$('#studentId').value=x.id;$('#studentShift').value=x.shift||'Matutino';$('#studentGroup').value=x.group||'';$('#studentNumber').value=x.number||'';$('#studentName').value=x.name||'';$('#studentBirthDay').value=x.birthDay||'';$('#studentBirthMonth').value=x.birthMonth||'';$('#studentObservations').value=x.observations||'';$('#studentIncidents').value=x.incidents||'';$('#studentTitle').textContent='Editar expediente';$('#cancelEdit').classList.remove('hidden')}
...
async function renderStudents(){
 let q=low($('#studentSearch')?.value||'');
 let groupFilter=$('#studentGroupFilter')?.value||'22';
 let allList=await allStudents();
 let active=allList.filter(x=>x.active!==false&&String(x.id)!=='00001');
 let inactive=allList.filter(x=>x.active===false&&String(x.id)!=='00001');

 const groupMatch=x=>groupFilter==='__ALL__'||sameGroup(x.group,groupFilter);
 const textMatch=x=>!q||[x.id,x.name,x.number,x.guardian,x.phone].some(v=>low(v).includes(q));

 let f=active.filter(x=>groupMatch(x)&&textMatch(x)),box=$('#studentsList');
 if(!f.length){
   box.className='list empty';
   box.textContent='No hay alumnos que coincidan con este grupo o búsqueda.';
 }else{
   box.className='list';
   box.innerHTML=f.sort(alpha).map(x=>`<div class="row"><div class="student-open" tabindex="0" role="button" data-profile="${safe(x.id)}"><strong>${safe(x.name||'Sin nombre')}</strong><small>ID ${safe(x.id)} · ${safe(x.shift)} · Grupo ${safe(x.group)} · Lista ${safe(x.number)}${x.guardian?` · Tutor: ${safe(x.guardian)}`:''}</small></div><div class="rowactions"><button class="edit" data-se="${safe(x.id)}">Editar</button><button class="danger-outline" data-deactivate-list="${safe(x.id)}">Dar de baja</button></div></div>`).join('');
 }

 let ib=$('#inactiveStudentsList');
 if(ib){
   let fi=inactive.filter(x=>groupMatch(x)&&textMatch(x));
   if(!fi.length){
     ib.className='list empty';
     ib.textContent='No hay alumnos inactivos en este grupo.';
   }else{
     ib.className='list';
     ib.innerHTML=fi.sort(alpha).map(x=>`<div class="row"><div class="student-open" tabindex="0" role="button" data-profile="${safe(x.id)}"><strong>${safe(x.name||'Sin nombre')} <span class="inactive-badge">INACTIVO</span></strong><small>ID ${safe(x.id)} · ${safe(x.shift)} · Grupo ${safe(x.group)} · ${safe(x.inactiveReason||'Baja del alumno')}</small></div><div class="rowactions"><button class="secondary" data-reactivate-list="${safe(x.id)}">Reactivar</button></div></div>`).join('');
   }
 }
 ...
 $$('[data-deactivate-list]').forEach(b=>b.onclick=()=>deactivateStudent(b.dataset.deactivateList));
 $$('[data-reactivate-list]').forEach(b=>b.onclick=()=>reactivateStudent(b.dataset.reactivateList));
}

async function deactivateStudent(id){
 let s=await req(store('students').get(id));if(!s)return;
 let reason=prompt('Motivo de la baja:',s.inactiveReason||'Baja del alumno');if(reason===null)return;
 if(!confirm(`¿Dar de baja a ${s.name||s.id}?\n\nSu historial NO se borrará. Ya no aparecerá en asistencia ni actividades y sus sesiones de los portales se cerrarán.`))return;
 try{
   if(supabaseReady)await ProfeSupabase.rpc('teacher_set_student_active',{p_student_id:String(id),p_active:false,p_reason:reason||'Baja del alumno'});
   s.active=false;s.inactiveAt=new Date().toISOString();s.inactiveReason=reason||'Baja del alumno';
   await req(store('students','readwrite').put(s));
   await fillSelectors();await renderStudents();await refreshDossierSelector(id);await refreshHome();await renderPortalAccessReport();
   alert('Alumno marcado como inactivo. Su expediente e historial se conservan.');
 }catch(e){alert('No se pudo dar de baja: '+(e.message||e))}
}
async function reactivateStudent(id){
 let s=await req(store('students').get(id));if(!s)return;
 if(!confirm(`¿Reactivar a ${s.name||s.id}?\n\nVolverá a aparecer en las listas normales.`))return;
 try{
   if(supabaseReady)await ProfeSupabase.rpc('teacher_set_student_active',{p_student_id:String(id),p_active:true,p_reason:null});
   s.active=true;s.inactiveAt='';s.inactiveReason='';
   await req(store('students','readwrite').put(s));
   await fillSelectors();await renderStudents();await refreshDossierSelector(id);await refreshHome();await renderPortalAccessReport();
   alert('Alumno reactivado. Si necesita acceso a Mi Español o Seguimiento Familiar, puedes generar un PIN temporal nuevo desde su expediente.');
 }catch(e){alert('No se pudo reactivar: '+(e.message||e))}
}
async function closeSchoolCycle(){
 if(!supabaseReady)return alert('Inicia sesión en Supabase antes de cerrar el ciclo escolar.');
 let active=(await allStudents()).filter(x=>x.active!==false);
 if(!active.length)return alert('No hay alumnos activos para cerrar.');
 const first=confirm(`Cerrar ciclo escolar dejará inactivos a ${active.length} alumnos.\n\nNO se borrarán calificaciones, entregas, asistencias, metodologías ni expedientes.\n\nPrimero se descargará un respaldo completo.`);
 if(!first)return;
 await exportBackup();
 const typed=prompt('Para confirmar el cierre del ciclo escribe CERRAR CICLO:');
 if(String(typed||'').trim().toUpperCase()!=='CERRAR CICLO')return alert('No se hizo ningún cambio.');
 try{
   let affected=await ProfeSupabase.rpc('teacher_close_school_cycle',{p_reason:'Cierre de ciclo escolar'});
   const now=new Date().toISOString();
   for(const s of active){s.active=false;s.inactiveAt=now;s.inactiveReason='Cierre de ciclo escolar';await req(store('students','readwrite').put(s))}
   await fillSelectors();await renderStudents();await refreshDossierSelector();await refreshHome();await renderPortalAccessReport();
   alert(`Ciclo cerrado. ${affected??active.length} alumnos quedaron inactivos. Todo el historial se conserva.`);
 }catch(e){alert('No se pudo cerrar el ciclo: '+(e.message||e))}
}
...
async function refreshDossierSelector(selectedId=null){
 if(!db)return;
 await pullStudentProfileFieldsFromSupabase();
 let list=await allStudents(),sel=$('#dossierStudent'),groupSel=$('#dossierGroup');
 if(!sel||!groupSel)return;

 // Si venimos desde la lista de alumnos, cambia automáticamente al grupo de ese alumno.
 if(selectedId){
   const selected=list.find(x=>x.id===selectedId);
   if(selected){
     const option=[...groupSel.options].find(o=>o.value!=='__ALL__'&&sameGroup(o.value,selected.group));
     if(option)groupSel.value=option.value;
     else groupSel.value='__ALL__';
   }
 }

 const group=groupSel.value||'22';
 const filtered=list
   .filter(x=>group==='__ALL__'||sameGroup(x.group,group))
   .sort(alpha);

 let old=selectedId||sel.value;
 sel.innerHTML=filtered.length
   ?'<option value="">Selecciona un alumno</option>'+filtered.map(x=>`<option value="${safe(x.id)}">${safe(x.name||x.id)}${x.active===false?' · INACTIVO':''} · ID ${safe(x.id)}</option>`).join('')
   :'<option value="">No hay alumnos en este grupo</option>';

 if(filtered.some(x=>x.id===old))sel.value=old;
 else sel.value='';

 await renderDossier(sel.value);
}
async function renderDossier(id){
 ...
 empty.classList.add('hidden');content.classList.remove('hidden');await renderPortalAccess(s);$('#dossierName').innerHTML=safe(s.name||s.id)+(s.active===false?' <span class="student-inactive-header">INACTIVO</span>':'');$('#dossierSchoolLine').textContent=`Grupo ${s.group} · No. de lista ${s.number} · ID ${s.id}`;let badge=$('#dossierShiftBadge');badge.textContent=s.shift||'Sin turno';badge.className='shift-badge '+(sameShift(s.shift,'Vespertino')?'vespertino':'');
 $('#dossierStudentInfo').innerHTML=`<dt>Nombre(s) y primer apellido</dt><dd>${safe(dossierValue(s.name))}</dd><dt>Turno</dt><dd>${safe(dossierValue(s.shift))}</dd><dt>Grupo</dt><dd>${safe(dossierValue(s.group))}</dd><dt>Número de lista</dt><dd>${safe(dossierValue(s.number))}</dd><dt>ID del alumno</dt><dd>${safe(dossierValue(s.id))}</dd><dt>Cumpleaños</dt><dd>${safe(birthdayLabel(s))}</dd>`;

 $('#dossierEdit').onclick=()=>editStudentById(s.id);$('#dossierPdf').onclick=()=>printStudentDossier(s.id);
 let deact=$('#dossierDeactivate'),react=$('#dossierReactivate');if(s.active===false){deact.classList.add('hidden');react.classList.remove('hidden');react.onclick=()=>reactivateStudent(s.id)}else{deact.classList.remove('hidden');react.classList.add('hidden');deact.onclick=()=>deactivateStudent(s.id)};
}
...
async function openGoogleChat(s){
 const email=prompt('Escribe el correo institucional para esta acción. No se guardará en el expediente.','')?.trim();
 if(!email)return;
 try{await navigator.clipboard.writeText(email)}catch(e){}
 alert('Correo copiado temporalmente. Pégalo en Google Chat.');
...
async function printStudentDossier(id){
 ...
 doc.autoTable({startY:40,body:[['Nombre(s) y primer apellido',s.name||'No registrado'],['Turno',s.shift||'No registrado'],['Grupo',String(s.group||'No registrado')],['Número de lista',String(s.number||'No registrado')],['ID del alumno',s.id||'No registrado'],['Cumpleaños',birthdayLabel(s)]],styles:{fontSize:9,cellPadding:2},columnStyles:{0:{fontStyle:'bold',cellWidth:55}},theme:'grid'});
 let y=doc.lastAutoTable.finalY+8;
...
async function snapshotData(){await ensureDB();let data={version:'8.12.3',created:new Date().toISOString()};for(let n of ['students','attendance','activities','activityRecords','titularWeeks','titularRecords','settings','methodologies','availability','notices','materials','studyTopics','studentMessages','portalReports'])if(db.objectStoreNames.contains(n))data[n]=await all(n);if(Array.isArray(data.students))data.students=data.students.map(s=>{const x={...s};delete x.email;delete x.guardian;delete x.phone;delete x.birthdate;delete x.tutor_name;delete x.tutor_phone;delete x.birth_date;delete x.tempPinReveal;return x});if(Array.isArray(data.studentMessages))data.studentMessages=data.studentMessages.filter(chatMessageIsCurrentWeek);delete data.portalAuth;return data}
...
window.addEventListener('load',()=>setTimeout(()=>startHomeScheduleWidget(),500));
document.addEventListener('visibilitychange',()=>{if(!document.hidden)startHomeScheduleWidget();});
