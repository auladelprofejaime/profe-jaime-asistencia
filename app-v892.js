...
async function resetSchoolCycle(){
 ...
 statusEl.textContent='Creando respaldo local antes de borrar...';

 try{
   // Respaldo obligatorio antes de limpiar.
   try{await exportBackup()}catch(e){console.warn('Respaldo previo',e)}

   statusEl.textContent='Eliminando datos de Supabase...';

   // Usa la sesión autenticada del docente para llamar al RPC seguro.
   const token=await ProfeSupabase.token();
   const response=await fetch(`${ProfeSupabase.SUPABASE_URL||'https://xqeyyjakmeiaahecfdmc.supabase.co'}/rest/v1/rpc/teacher_reset_school_data`,{
     method:'POST',
     headers:{
       apikey:ProfeSupabase.SUPABASE_PUBLISHABLE_KEY||'sb_publishable_GY2NGAigumnZw3rIJKU7LA_a2qigAEA',
   ...
   statusEl.textContent='Limpiando datos de este iPad...';
   await clearLocalSchoolData();

   // Reinicia controles y vistas sin borrar configuración técnica.
   try{
     await refreshAll();
     await refreshHome();
   ...
   console.error(e);
   statusEl.textContent='No se pudo completar el reinicio: '+(e.message||e);
   alert('No se borró todo. Revisa el mensaje de error antes de volver a intentarlo.');
 }finally{
...
function bind(){
 ...
 $('#cancelResetCycle')&&($('#cancelResetCycle').onclick=()=>$('#resetCycleDialog')?.close());
 $('#resetCycleConfirmText')&&($('#resetCycleConfirmText').oninput=e=>{$('#confirmResetCycle').disabled=e.target.value.trim()!=='BORRAR TODO'});
 $('#confirmResetCycle')&&($('#confirmResetCycle').onclick=resetSchoolCycle);
