...
function remoteStudent(s){return {id:String(s.id),name:s.name||String(s.id),shift:s.shift||'',group_name:s.group||'',list_number:Number(s.number)||null,institutional_email:s.email||null,tutor_name:s.guardian||null,tutor_phone:s.phone||null,birth_date:s.birthdate||null,observations:s.observations||null,incidents:s.incidents||null,active:s.active!==false,inactive_at:s.inactiveAt||null,inactive_reason:s.inactiveReason||null}}
...
async function pullStudentProfileFieldsFromSupabase(){
 if(!supabaseReady||!window.ProfeSupabase)return;
 try{
   const rows=await ProfeSupabase.select('students','select=id,tutor_name,tutor_phone,birth_date&active=eq.true');
   for(const r of rows||[]){
     const local=await req(store('students').get(String(r.id)));
     if(!local)continue;
     let changed=false;
     if(r.tutor_name && local.guardian!==r.tutor_name){local.guardian=r.tutor_name;changed=true}
     if(r.tutor_phone && local.phone!==r.tutor_phone){local.phone=r.tutor_phone;changed=true}
     if(r.birth_date && local.birthdate!==r.birth_date){local.birthdate=r.birth_date;changed=true}
     if(changed)await req(store('students','readwrite').put(local));
   }
 }catch(e){console.warn('No se pudieron actualizar datos familiares',e)}
}
...
function renderWeeklyBirthdays(list){
 ...
 for(const d of days){
   ...
   const people=list.filter(s=>birthMonthDay(s.birthdate)===key).sort(alpha);
...
async function createActivity(e){e.preventDefault();let editId=$('#editActivityId').value,old=editId?await req(store('activities').get(editId)):null,newMode=$('#newActEvaluation').value||'delivery',dueDate=$('#newActDueDate').value;if(!dueDate){alert('Selecciona la fecha de entrega.');$('#newActDueDate').focus();return}if(old&&(old.evaluationMode||'delivery')!==newMode){let existing=(await all('activityRecords')).some(r=>r.activityId===old.id);if(existing){alert('No se puede cambiar la forma de evaluación porque esta actividad ya tiene registros. Puedes editar nombre, fechas, semana, tipo, turno o grupo sin perderlos.');$('#newActEvaluation').value=old.evaluationMode||'delivery';return}}let v={id:editId||crypto.randomUUID(),shift:$('#newActShift').value,group:$('#newActGroup').value,date:$('#newActDate').value,dueDate,week:norm($('#newActWeek').value),name:norm($('#newActName').value),type:$('#newActType').value,evaluationMode:newMode,order:old?.order??Date.now(),created:old?.created||new Date().toISOString(),updated:new Date().toISOString()};await put('activities',v);if(sameShift(v.shift,'Matutino'))await sendTeacherPushEvent('new_activity',{title:v.name,group:v.group||null});resetActivityForm();await refreshActivitySelectors();await refreshGridWeeks();await renderActivities();await refreshCalculationMethodologies();alert(editId?'Actividad actualizada. Los registros se conservaron.':'Actividad guardada')}
...
async function renderStudents(){
 let q=low($('#studentSearch').value),allList=await allStudents(),active=allList.filter(x=>x.active!==false),inactive=allList.filter(x=>x.active===false);
 let f=active.filter(x=>!q||[x.id,x.shift,x.group,x.number,x.name,x.email,x.guardian,x.phone].some(v=>low(v).includes(q))),box=$('#studentsList');
 if(!f.length){box.className='list empty';box.textContent='No hay alumnos activos.'}else{
 ...
 let ib=$('#inactiveStudentsList');if(ib){
   let fi=inactive.filter(x=>!q||[x.id,x.shift,x.group,x.number,x.name,x.email,x.guardian,x.phone].some(v=>low(v).includes(q)));
   if(!fi.length){ib.className='list empty';ib.textContent='No hay alumnos inactivos.'}else{
     ib.className='list';ib.innerHTML=fi.map(x=>`<div class="row"><div class="student-open" tabindex="0" role="button" data-profile="${safe(x.id)}"><strong>${safe(x.name||'Sin nombre')} <span class="inactive-badge">INACTIVO</span></strong><small>ID ${safe(x.id)} · ${safe(x.shift)} · Grupo ${safe(x.group)} · ${safe(x.inactiveReason||'Baja del alumno')}</small></div><div class="rowactions"><button class="secondary" data-reactivate-list="${safe(x.id)}">Reactivar</button></div></div>`).join('');
   }
 }
...
async function refreshDossierSelector(selectedId=null){if(!db)return;let list=await allStudents(),sel=$('#dossierStudent');if(!sel)return;let old=selectedId||sel.value;sel.innerHTML=list.length?'<option value="">Selecciona un alumno</option>'+list.map(x=>`<option value="${safe(x.id)}">${safe(x.name||x.id)}${x.active===false?' · INACTIVO':''} · ${safe(x.group)} · ${safe(x.shift)}</option>`).join(''):'<option value="">No hay alumnos</option>';if(list.some(x=>x.id===old))sel.value=old;await renderDossier(sel.value)}
...
async function renderPortalAccessReport(){
 ...
 const students=(await all('students')).filter(s=>s.active!==false&&sameShift(s.shift,'Matutino')).sort((a,b)=>(a.name||'').localeCompare(b.name||'','es'));
 ...
 for(const s of students){
   ...
   rows.push(`<tr><td><b>${safe(s.name||'')}</b><small>${safe(s.group||'')} · ID ${safe(s.id)}</small></td><td>${portalStatePill(st.student)}${st.student?.mustChange&&st.student?.tempPinReveal?`<div class="pin-mini">${safe(st.student.tempPinReveal)}</div>`:''}</td><td>${portalStatePill(st.parent)}${st.parent?.mustChange&&st.parent?.tempPinReveal?`<div class="pin-mini">${safe(st.parent.tempPinReveal)}</div>`:''}</td></tr>`);
 }
 sum.innerHTML=`
...
async function checkTeacherMessageNotifications(initial=false){
 if(!supabaseReady)return;
 try{
   const remote=await ProfeSupabase.select('student_messages','select=*&order=sent_at.desc&limit=100');
...
async function pullRemoteStudentMessages(){
 if(!supabaseReady)return;
 try{
   const remote=await ProfeSupabase.select('student_messages','select=*&order=sent_at.desc');
...
async function resetSchoolCycle(){
 const input=$('#resetCycleConfirmText');
 const statusEl=$('#resetCycleStatus');
 const confirmBtn=$('#confirmResetCycle');
 const cancelBtn=$('#cancelResetCycle');

 if(!input||input.value.trim()!=='BORRAR TODO')return;

 confirmBtn.disabled=true;
 if(cancelBtn)cancelBtn.disabled=true;
 statusEl.textContent='Preparando reinicio...';

 try{
   // No se descarga respaldo automáticamente: en iPad/PWA puede bloquear el flujo.
   statusEl.textContent='1/3 · Eliminando datos de Supabase...';
   ...
   statusEl.textContent='2/3 · Limpiando datos de este iPad...';
   await clearLocalSchoolData();

   statusEl.textContent='3/3 · Comprobando que quedó en cero...';
   const remaining=await all('students');

   if(remaining.length){
     throw new Error(`Quedaron ${remaining.length} alumnos en el almacenamiento local.`);
   }

   try{
     await refreshAll();
     await refreshHome();
   }catch(e){console.warn('Refresh post reset',e)}

   statusEl.textContent='✅ Sistema reiniciado. 0 alumnos registrados.';
   await new Promise(r=>setTimeout(r,700));

   try{$('#resetCycleDialog')?.close()}catch(_){}
   input.value='';
   confirmBtn.disabled=true;
   if(cancelBtn)cancelBtn.disabled=false;
   setView('home');

   alert('✅ Sistema reiniciado correctamente.\n\nYa puedes cargar los alumnos reales del nuevo ciclo.');

 }catch(e){
   console.error(e);
   statusEl.textContent=e?.name==='AbortError'
     ? 'El reinicio tardó demasiado y se canceló. No continúes sin revisar Supabase.'
     : 'No se pudo completar el reinicio: '+(e.message||e);

   confirmBtn.disabled=false;
   if(cancelBtn)cancelBtn.disabled=false;
 }
}
function openResetCycleDialog(){
 const d=$('#resetCycleDialog');
 $('#resetCycleConfirmText').value='';
 $('#resetCycleStatus').textContent='';
 $('#confirmResetCycle').disabled=true;
 if($('#cancelResetCycle'))$('#cancelResetCycle').disabled=false;
 if(typeof d.showModal==='function')d.showModal();
 else d.setAttribute('open','');
}


async function handleResetCycleClick(){
 const statusEl=$('#resetCycleStatus');
 const input=$('#resetCycleConfirmText');
 if(!input||input.value.trim()!=='BORRAR TODO'){
   if(statusEl)statusEl.textContent='Escribe exactamente BORRAR TODO.';
   return;
 }
 if(statusEl)statusEl.textContent='Iniciando reinicio...';
 await new Promise(r=>setTimeout(r,50));
 return resetSchoolCycle();
}

function bind(){
 $('#resetCycleBtn')&&($('#resetCycleBtn').onclick=openResetCycleDialog);
 $('#cancelResetCycle')&&($('#cancelResetCycle').onclick=()=>{
   try{$('#resetCycleDialog')?.close()}catch(_){$('#resetCycleDialog')?.removeAttribute('open')}
 });
 $('#resetCycleConfirmText')&&($('#resetCycleConfirmText').oninput=e=>{$('#confirmResetCycle').disabled=e.target.value.trim()!=='BORRAR TODO';$('#resetCycleStatus').textContent=''});
 $('#confirmResetCycle')&&($('#confirmResetCycle').onclick=handleResetCycleClick);
...
$('#availabilityForm').onsubmit=saveAvailability;
 $('#contactOverrideBtn').onclick=toggleContactOverride;$('#noticeForm').onsubmit=e=>savePortalEntity('notice',e);$('#materialForm').onsubmit=e=>savePortalEntity('material',e);$('#topicForm').onsubmit=e=>savePortalEntity('topic',e);$('#refreshMessages').onclick=renderTeacherMessages;
