...
async function renderStudents(){
 ...
 let active=allList.filter(x=>x.active!==false);
 let inactive=allList.filter(x=>x.active===false);
...
async function resetSchoolCycle(){
 ...
 try{
   ...
   if(statusEl)statusEl.textContent='3/3 · Comprobando que quedó en cero...';
   ...
   if(statusEl)statusEl.textContent='✅ Sistema reiniciado. 0 alumnos registrados.';
...
async function promptAndResetSchoolCycle(){
 const typed=window.prompt(
   '⚠️ REINICIAR DATOS DE PRUEBA\n\nSe eliminarán alumnos, asistencias, actividades, calificaciones, mensajes, reportes, sesiones de alumno/padre y suscripciones push.\n\nNo se borrará el código ni la configuración técnica.\n\nEscribe exactamente BORRAR TODO para continuar:'
 );
 ...
 const sure=window.confirm(
   'ÚLTIMA CONFIRMACIÓN\n\nEsta acción es permanente para los datos de prueba.\n\n¿Eliminar todo ahora?'
 );
 if(!sure)return;
