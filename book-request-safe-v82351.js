...
(function(){
  ...
  async function safeRequestAllLiquidated(){
    ...
    let estimated=0;
    try{
      const rows=bookPayDashboard?.students||[];
      estimated=rows.filter(r=>r.status==='paid'&&!r.fulfillment_status).length;
    }catch(_){}
    ...
    if(!confirm('¿Marcar como solicitados todos los libros liquidados pendientes?\n\nSe detectan aproximadamente '+estimated+' libro(s). Esta operación no modifica pagos ni saldos.'))return;
    ...
    try{
      const out=await directRpc('teacher_book_request_all_liquidated',{});
      if(!out?.ok)throw new Error(out?.reason||'No se pudo completar la solicitud.');

      const ids=Array.isArray(out.student_ids)?out.student_ids:[];
      setRequestedLocally(ids);

      if(Number(out.updated||0)>0){
        alert('Listo. '+Number(out.updated)+' libro(s) quedaron marcados como solicitados.');
      }else{
        alert('No había libros liquidados pendientes de solicitar. El estado ya estaba actualizado.');
      }

      // Refresco diferido y silencioso; no hace fallar la acción ya confirmada.
      setTimeout(()=>{
        try{
          if(navigator.onLine&&typeof loadBookFulfillment==='function')loadBookFulfillment().catch(()=>{});
        }catch(_){}
      },8000);
    }catch(e){
      if(is429(e)){
        alert('Supabase está aplicando un límite temporal de solicitudes. No puedo confirmar todavía si la operación entró. Espera a que el indicador vuelva a verde y vuelve a tocar este mismo botón. Es seguro repetirlo: no duplica solicitudes ni modifica pagos.');
      }else{
        alert('No se pudo confirmar la solicitud de libros: '+(e?.message||e));
      }
    }finally{
