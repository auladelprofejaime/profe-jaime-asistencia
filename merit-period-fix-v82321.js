// Hotfix v8.23.21 · visualización robusta de periodos Mérito Gabino A. Palma
(function(){
  let meritPeriodRetryTimer=null;
  function scheduleMeritPeriodRetry(){
    clearTimeout(meritPeriodRetryTimer);
    meritPeriodRetryTimer=setTimeout(()=>refreshMeritPeriodsHotfix().catch(()=>{}),1200);
  }
  ...
  async function refreshMeritPeriodsHotfix(){
    try{
      ...
      const box=document.getElementById('meritPeriodsList');
      if(/sesión requerida|No autorizado|auth|session/i.test(msg)){
        if(box)box.innerHTML='<p class="hint">Conectando con la sesión del profesor…</p>';
        scheduleMeritPeriodRetry();
      }else if(box){
        box.innerHTML='<p class="message">No se pudieron cargar los periodos: '+safe(msg)+'</p>';
      }
      throw e;
    }
  }
  ...
  const wire=()=>{
    ...
    document.querySelectorAll('.meritNav').forEach(b=>{
      b.addEventListener('click',()=>setTimeout(()=>refreshMeritPeriodsHotfix().catch(()=>{}),80));
    });
    window.addEventListener('focus',()=>refreshMeritPeriodsHotfix().catch(()=>{}));
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')refreshMeritPeriodsHotfix().catch(()=>{})});
    [700,1800,3500].forEach(ms=>setTimeout(()=>refreshMeritPeriodsHotfix().catch(()=>{}),ms));
  };
