...
(function(){
  ...
  async function loadPeriods(){
    ...
    try{
      ...
      const data=(Array.isArray(rows)?rows:[]).filter(p=>!(String(p.starts_at||'')===String(p.ends_at||'') || /^Prueba\b/i.test(String(p.label||''))));
      const old=sel.value;
      ...
      if(data.some(p=>String(p.id)===String(old)))sel.value=old;
        ...
        const ordered=[...data].sort((a,b)=>String(a.starts_at||'').localeCompare(String(b.starts_at||'')));
        const official=ordered.find(p=>String(p.starts_at||'')>=todayIso) || ordered[ordered.length-1];
  ...
  async function activateForSelectedPeriod(e){
    ...
    if(!periodId){status.textContent='No hay un periodo oficial disponible para activar la participación.';return}
