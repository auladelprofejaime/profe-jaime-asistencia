// App Docente v8.23.15 · Folios de docentes para Mérito Gabino A. Palma
(function(){
  ...
  async function findTeacherFolio(){
    ...
    if(!/^MGP-D\d{3}$/.test(code)){
      if(status)status.textContent='Escanea o escribe un folio válido, por ejemplo MGP-D001.';
    ...
    try{
      ...
      if(d.assigned){
        ...
        status.textContent='Folio disponible. Escribe los datos del docente para activarlo.';
  ...
  async function activateTeacherFolio(){
    ...
    try{
      ...
      const codeBox=$m('#meritActivationCode');
      if(codeBox)codeBox.textContent=d.activation_code;
      box?.classList.remove('hidden');
      status.textContent=`✓ ${name} quedó activo con el folio ${code}.`;
  ...
  function printTeacherFolios(){
    ...
    const cards=Array.from({length:60},(_,i)=>{
      ...
      return `<section class="folio">
        ...
        <div class="note"><b>Este folio todavía no está asignado.</b><br>Si deseas participar en el proyecto, preséntalo al Profr. Jaime para activarlo. En ese momento se te entregará tu código personal de 4 dígitos para activar la app Mérito Docentes.</div>
        <div class="steps"><b>1.</b> Conserva este folio. &nbsp; <b>2.</b> Se escanea y registra tu nombre. &nbsp; <b>3.</b> Recibes tu código de activación.</div>
      </section>`;
    }).join('');
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Folios docentes · Mérito Gabino A. Palma</title><style>
      ...
      .note{font-size:11pt;line-height:1.45;background:#fff8d7;border-radius:10px;padding:5mm}.steps{font-size:9.5pt;margin-top:5mm;line-height:1.5}
