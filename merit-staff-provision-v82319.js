// App Docente v8.23.19 · Gestión de docentes de Mérito
(function(){
  ...
  async function printTeacherFolios(){
    ...
    const meritLogo=new URL('logo-merito-gabino-a-palma.jpeg',location.href).href;
    const cards=credentials.map(item=>{
      const code=String(item.staff_code||''),pin=String(item.pin||'');
      return `<section class="folio"><div class="top"><div class="brandhead"><img class="brand-logo" src="${meritLogo}" alt="Mérito Gabino A. Palma"><div><div class="eyebrow">MÉRITO GABINO A. PALMA</div><h1>Acceso docente</h1><div class="byline">Una app de Aula del Profe Jaime</div></div></div><div class="id">${code}</div></div><div class="barcode">${code39Svg(code)}</div><div class="credentials"><div><span>ID DOCENTE</span><b>${code}</b></div><div><span>NIP INICIAL</span><b>${pin}</b></div></div><div class="note"><b>Acceso de prueba a Mérito Docentes.</b><br>Ingresa con este ID y NIP. Si confirmas tu participación, el Profr. Jaime activará tu ID y la app te pedirá completar tus datos y elegir un NIP personal.</div><div class="steps"><b>1.</b> Ingresa con ID + NIP. &nbsp; <b>2.</b> Prueba la app. &nbsp; <b>3.</b> Si participas, solicita la activación de tu ID.</div><div class="brandfoot">Aula del Profe Jaime · Creado por Profesor Jaime Armando</div></section>`;
    }).join('');
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Hojas de acceso docentes · Mérito Gabino A. Palma</title><style>@page{size:letter;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;margin:0;color:#211b12}.folio{height:126mm;border:1.5px solid #d7b11e;border-radius:14px;padding:10mm;margin:0 0 8mm;page-break-inside:avoid;background:#fff}.folio:nth-child(2n){page-break-after:always}.top{display:flex;justify-content:space-between;gap:10mm;align-items:flex-start}.brandhead{display:flex;align-items:center;gap:5mm}.brand-logo{width:22mm;height:22mm;object-fit:contain}.eyebrow{font-size:9pt;font-weight:800;letter-spacing:.08em;color:#806800}.top h1{margin:2mm 0 0;font-size:20pt}.byline{font-size:8.5pt;font-weight:700;margin-top:1.5mm;opacity:.72}.id{font-size:18pt;font-weight:900;border:2px solid #211b12;border-radius:10px;padding:3mm 5mm}.barcode{margin:8mm auto 5mm;max-width:155mm}.credentials{display:grid;grid-template-columns:1fr 1fr;gap:5mm;margin:4mm 0}.credentials div{border:1px solid #d7b11e;border-radius:10px;padding:4mm;text-align:center}.credentials span{display:block;font-size:8pt;font-weight:800;letter-spacing:.08em;color:#806800}.credentials b{display:block;font-size:20pt;margin-top:1mm}.note{font-size:11pt;line-height:1.45;background:#fff8d7;border-radius:10px;padding:5mm}.steps{font-size:9.5pt;margin-top:5mm;line-height:1.5}.brandfoot{margin-top:5mm;padding-top:3mm;border-top:1px solid #ddd;text-align:center;font-size:8.5pt;font-weight:700;opacity:.72}</style></head><body>${cards}</body></html>`);
