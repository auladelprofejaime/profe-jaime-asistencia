...
function renderMaterials(){$('#materialCards').innerHTML=bundle.materials.length?bundle.materials.map(m=>`<a class="card action" href="${esc(m.url)}" target="_blank"><b>${esc(m.title)}</b><p class="muted">${esc(m.type)}</p></a>`).join(''):'<div class="card muted">Sin materiales publicados.</div>'}
