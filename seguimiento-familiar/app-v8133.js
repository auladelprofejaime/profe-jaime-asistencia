...
function familyChange(v,suffix=""){
 const n=Number(v);
...
function familyFollowupCard(f){
 ...
 const r=f.reading||{}, w=f.writing||{};
 const elChange=Number(r.change?.el);
 const writingChange=Number(w.change?.writing_total);

 let readingText="La comparación muestra cambios entre el punto de partida y la aplicación final.";
 if(Number.isFinite(elChange)){
   if(elChange>0)readingText="Se observa un avance en la eficacia lectora respecto al punto de partida.";
   else if(elChange<0)readingText="La aplicación final mostró un resultado menor en eficacia lectora; conviene interpretarlo junto con el trabajo realizado y las condiciones de aplicación.";
   else readingText="La eficacia lectora se mantuvo estable entre ambas aplicaciones.";
 }

 let writingText="La escritura cuenta con una comparación entre la aplicación inicial y final.";
 if(Number.isFinite(writingChange)){
   if(writingChange>0)writingText="Se observa avance en el resultado global de redacción.";
   else if(writingChange<0)writingText="El resultado global de redacción fue menor en la aplicación final; conviene continuar el acompañamiento.";
   else writingText="El resultado global de redacción se mantuvo estable.";
 }

 return `<div class="card familyFollowupCard">
   <h3>Avance durante el ciclo escolar</h3>
   <p>${esc(readingText)}</p>
   <div class="familyFollowupMetrics">
     <div><b>Lectura</b><span>EL ${esc(r.initial?.el??"—")} → ${esc(r.final?.el??"—")}</span><small>Cambio: ${esc(familyChange(r.change?.el))}</small></div>
     <div><b>Escritura</b><span>Redacción ${esc(w.initial?.writing_total??"—")} → ${esc(w.final?.writing_total??"—")}</span><small>Cambio: ${esc(familyChange(w.change?.writing_total))}</small></div>
   </div>
   <p>${esc(writingText)}</p>
   <p class="muted">Esta comparación es un seguimiento pedagógico y no una calificación.</p>
 </div>`;
}
...
async function ensureFamilyServiceWorker(){
 ...
 familySWRegistration=await navigator.serviceWorker.register('./service-worker.js?v=8133',{scope:'./'});
