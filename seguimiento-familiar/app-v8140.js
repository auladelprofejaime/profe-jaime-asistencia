...
let parentWhatsappState=null;
let parentWhatsappOpened=false;
let parentWhatsappResolved=true;

async function parentWhatsappRpc(fn,payload={}){
  const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/${fn}`,{
    method:'POST',
    headers:{apikey:SUPABASE_PUBLISHABLE_KEY,'Content-Type':'application/json'},
    body:JSON.stringify(payload)
  });
  if(!r.ok)throw new Error(`No se pudo consultar ${fn}.`);
  return await r.json();
}
function setParentWhatsappStep(step){
  const modal=$('#whatsappOnboarding');
  if(!modal)return;
  modal.classList.remove('hidden');
  document.body.classList.add('whatsapp-onboarding-lock');
  $('#whatsappInviteStep')?.classList.toggle('hidden',step!=='invite');
  $('#whatsappConfirmStep')?.classList.toggle('hidden',step!=='confirm');
  $('#whatsappDoneStep')?.classList.toggle('hidden',step!=='done');
}
function closeParentWhatsappOnboarding(){
  $('#whatsappOnboarding')?.classList.add('hidden');
  document.body.classList.remove('whatsapp-onboarding-lock');
  parentWhatsappResolved=true;
  parentWhatsappOpened=false;
}
async function checkParentWhatsappOnboarding(){
  if(!currentToken)return true;
  try{
    const state=await parentWhatsappRpc('portal_parent_whatsapp_status',{p_token:currentToken});
    parentWhatsappState=state;
    if(!state?.ok||!state.required){
      closeParentWhatsappOnboarding();
      return true;
    }
    parentWhatsappResolved=false;
    const grp=String(state.group_name||'').trim();
    const version=String(state.invite_version||'');
    const markerKey=`parentWhatsappOpened:${id}:${grp}:${version}`;
    parentWhatsappOpened=sessionStorage.getItem(markerKey)==='1';
    const label=grp==='__MONITOR_TEST__'?'Prueba del Monitor':(grp?`Grupo ${grp}`:'');
    if($('#whatsappOnboardingGroup'))$('#whatsappOnboardingGroup').textContent=label;
    setParentWhatsappStep(parentWhatsappOpened?'confirm':'invite');
    return false;
  }catch(e){
    console.warn('WhatsApp onboarding',e);
    closeParentWhatsappOnboarding();
    return true;
  }
}
function openParentWhatsappInvite(){
  const url=String(parentWhatsappState?.invite_url||'').trim();
  if(!url)return;
  parentWhatsappOpened=true;
  const grp=String(parentWhatsappState?.group_name||'').trim();
  const version=String(parentWhatsappState?.invite_version||'');
  sessionStorage.setItem(`parentWhatsappOpened:${id}:${grp}:${version}`,'1');
  try{window.location.href=url}catch(e){window.open(url,'_blank')}
}
function handleParentWhatsappReturn(){
  if(!parentWhatsappResolved&&parentWhatsappOpened)setParentWhatsappStep('confirm');
}
async function confirmParentWhatsappJoined(){
  const btn=$('#whatsappConfirmBtn');
  if(btn){btn.disabled=true;btn.textContent='Guardando…';}
  try{
    const r=await parentWhatsappRpc('portal_parent_whatsapp_confirm',{p_token:currentToken});
    if(!r?.ok)throw new Error('No se pudo guardar la confirmación.');
    const grp=String(parentWhatsappState?.group_name||'').trim();
    const version=String(parentWhatsappState?.invite_version||'');
    sessionStorage.removeItem(`parentWhatsappOpened:${id}:${grp}:${version}`);
    setParentWhatsappStep('done');
  }catch(e){
    alert(e.message||'No se pudo guardar la confirmación.');
  }finally{
    if(btn){btn.disabled=false;btn.textContent='Sí, ya me uní';}
  }
}
...
document.addEventListener('visibilitychange',async()=>{
 if(document.visibilityState==='visible')handleParentWhatsappReturn();
...
async function openPortalView(id){
 ...
 if(id==='diagnostic'&&currentToken){
   try{
     await renderPublishedDiagnostic();
   }catch(e){
     console.error('No se pudo abrir Diagnóstico inicial',e);
     const box=$('#diagnosticResult');
     if(box){
       box.innerHTML=`<div class="card"><b>No se pudo abrir el diagnóstico.</b><p class="muted">${esc(e?.message||String(e))}</p></div>`;
     }
   }
   return;
 }
...
async function getPublishedDiagnostic(){
 ...
 try{
  ...
  return await r.json();
   ...
   console.warn('diagnostic result',e);
   throw e;
 }
}
function diagFriendlyArea(raw){
 ...
 if(key.includes("complec")||key.includes("lectura eficaz")) return "Comprensión y eficacia lectora";
...
function familyReadingInterpretation(f){
 if(!f?.comparison_available)return "";
 const r=f.reading||{};
 ...
 }else opening="La comparación permite observar la evolución de la lectura durante el ciclo escolar.";
...
function familyWritingInterpretation(f){
 if(!f?.comparison_available)return "";
 const w=f.writing||{}, ch=w.change||{};
 const vals=[
   Number(ch.writing_total),
   Number(ch.words_score),
   Number(ch.pseudowords_score),
   Number(ch.orthographic_rules_score),
   Number(ch.accents_score),
   Number(ch.capitals_score),
   Number(ch.punctuation_score)
 ...
 const improvements=[];
 if(Number(ch.writing_total)>0)improvements.push("la redacción");
 if(Number(ch.words_score)>0)improvements.push("la escritura correcta de palabras de uso frecuente");
 if(Number(ch.orthographic_rules_score)>0)improvements.push("la aplicación de reglas ortográficas");
 if(Number(ch.accents_score)>0)improvements.push("el uso de acentos");
 if(Number(ch.capitals_score)>0)improvements.push("el uso de mayúsculas");
 if(Number(ch.punctuation_score)>0)improvements.push("la puntuación");
 ...
 const rec=negatives>0
   ?"Conviene continuar con práctica guiada de escritura y revisión de errores, dando especial atención a los aspectos que todavía muestran dificultad."
   :"Es recomendable continuar reforzando la revisión de textos, ortografía y puntuación para consolidar los progresos observados.";

 return `${opening} ${detail} ${rec}`.replace(/\s+/g," ").trim();
}

function familyFollowupCard(f){
 if(!f?.comparison_available)return "";
 return `<div class="card familyFollowupCard">
   <h3>Avance durante el ciclo escolar</h3>
   <div class="familyFollowupNarrative">
     <div><b>Lectura</b><p>${esc(familyReadingInterpretation(f))}</p></div>
     <div><b>Escritura</b><p>${esc(familyWritingInterpretation(f))}</p></div>
   </div>
   <p class="muted">Este seguimiento es pedagógico y busca orientar el acompañamiento; no representa una calificación.</p>
 </div>`;
}
...
async function renderPublishedDiagnostic(){
 ...
 const reading=tests?.lectura_eficaz||null;
 const followup=r.followup||null;
 const readingCard=reading?`
   <div class="card">
     <h3>Lectura</h3>
     <p><b>${esc(reading.level||"Resultado disponible")}</b></p>
     <p>La lectura inicial permitió observar de manera conjunta la fluidez y la comprensión. Este dato se utilizará como punto de partida para valorar el avance durante el ciclo escolar.</p>
   </div>`:"";
...
async function init(){
 ...
 try{
   ...
   $('#whatsappJoinBtn')&&($('#whatsappJoinBtn').onclick=openParentWhatsappInvite);
   $('#whatsappConfirmBtn')&&($('#whatsappConfirmBtn').onclick=confirmParentWhatsappJoined);
   $('#whatsappContinueBtn')&&($('#whatsappContinueBtn').onclick=closeParentWhatsappOnboarding);
   window.addEventListener('focus',handleParentWhatsappReturn);
...
async function enterPortal(){
 try{
   ...
   await load();
   await checkParentWhatsappOnboarding();
   $('#sessionLoading')?.classList.add('hidden');
   $('#loginGate')?.classList.add('hidden');
   $('#portalApp')?.classList.remove('hidden');
   return true;
